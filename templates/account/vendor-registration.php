...
<form id="dokan-vendor-register" method="post" class="register dokan-vendor-register">
    ...
    <?php if ( 'no' === get_option( 'woocommerce_registration_generate_username' ) ) : ?>
        <p class="form-row form-group form-row-wide">
            ...
            <input type="text" class="input-text form-control" name="username" id="reg_username" value="<?php echo ! empty( $postdata['username'] ) ? esc_attr( $postdata['username'] ) : ''; ?>" required="required" />
        </p>
    <?php endif; ?>

    <p class="form-row form-group form-row-wide">
        <label for="reg_email"><?php esc_html_e( 'Email address', 'dokan-lite' ); ?> <span class="required">*</span></label>
        <input type="email" class="input-text form-control" name="email" id="reg_email" value="<?php echo ! empty( $postdata['email'] ) ? esc_attr( $postdata['email'] ) : ''; ?>" required="required" />
    ...
    <p class="form-row form-group form-row-wide">
        ...
        <input type="password" class="input-text form-control" name="password" id="reg_password" value="<?php echo ! empty( $postdata['password'] ) ? esc_attr( $postdata['password'] ) : ''; ?>" required="required" minlength="6" />
    </p>
        ...
        <p class="form-row form-group form-row-wide">
            ...
            <label style="display: inline" for="tc_agree">
                ...
                printf(
                    ...
                    __( 'I have read and agree to the %1$sTerms %2$s Conditions%2$s.', 'dokan-lite' ),
