            ...
            <tbody><?php
                ...
                foreach ( $customer_orders as $customer_order ) {
                    ...
                    ?><tr class="order">
                        ...
                        <td class="order-total">
                            <?php
                                $seller_id = dokan_get_seller_id_by_order( dokan_cmp_get_prop( $order, 'id' ) );
                        ...
                        <td class="order-actions">
                            <?php
                                ...
                                if ( in_array( dokan_cmp_get_prop( $order, 'status' ), apply_filters( 'woocommerce_valid_order_statuses_for_payment', array( 'pending', 'failed' ), $order ) ) )
                                    $actions['pay'] = array(
                                        'url'  => $order->get_checkout_payment_url(),
                                        'name' => __( 'Pay', 'dokan-lite' )
                                    );

                                if ( in_array( dokan_cmp_get_prop( $order, 'status' ), apply_filters( 'woocommerce_valid_order_statuses_for_cancel', array( 'pending', 'failed' ), $order ) ) )
