...
test.describe.skip('calculation test', () => {
    ...
    test('calculation test', async ({ request }) => {
        ...
        let discountTotal, discountTax, shippingTotal, shippingTax, cartTax, totalTax, orderTotal, paymentMethod, productPrice, productQuantity, gatewayFee = 0
        ...
        let calculatedSubTotal = helpers.subtotal([productPrice], [productQuantity])
        let calculatedTax = helpers.tax(taxRate, calculatedSubTotal, shippingTotal)
        let calculatedOrderTotal = helpers.orderTotal(calculatedSubTotal, calculatedTax, shippingTotal)
        let calculatedAdminCommission = helpers.adminCommission(calculatedSubTotal, commissionRate, calculatedTax, shippingTotal, gatewayFee)
        let calculatedVendorEarning = helpers.vendorEarning(calculatedSubTotal, calculatedAdminCommission, calculatedTax, shippingTotal, gatewayFee)
        console.log(
