...
test.describe('stores api test', () => {
    ...
    test('admin email store', async ({ request }) => {
        ...
        let response = await request.post(endPoints.adminEmailStore(sellerId), { data: payloads.adminEmailStore })
        let responseBody = await apiUtils.getResponseBody(response)
        expect(response.ok()).toBeTruthy()
    });

    test('update batch stores', async ({ request }) => {
        let allStoreIds = (await apiUtils.getAllStores()).map((a: { id: any }) => a.id)
