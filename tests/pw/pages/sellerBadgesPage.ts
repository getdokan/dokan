...
export class SellerBadgesPage extends AdminPage {
    ...
    async adminSellerBadgeRenderProperly() {
        ...
        await this.toBeVisible(
            selector.admin.dokan.sellerBadge.sellerBadgeText,
        );
        ...
        await this.multipleElementVisible(
            selector.admin.dokan.sellerBadge.navTabs,
        );

        // bulk action elements are visible
        await this.multipleElementVisible(
            selector.admin.dokan.sellerBadge.bulkActions,
        );
        ...
        await this.multipleElementVisible(
            selector.admin.dokan.sellerBadge.table,
        );
    }
    ...
    async searchSellerBadge(badgeName: string) {
        ...
        await this.typeAndWaitForResponseAndLoadState(
            data.subUrls.api.dokan.sellerBadge,
            selector.admin.dokan.sellerBadge.search,
            badgeName,
        );
        await this.toBeVisible(
            selector.admin.dokan.sellerBadge.sellerBadgeCell(badgeName),
        );
    }
    ...
    async viewSellerBadge(badgeName: string) {
        ...
        await this.hover(
            selector.admin.dokan.sellerBadge.sellerBadgeRow(badgeName),
        );
        await this.clickAndWaitForResponse(
            data.subUrls.api.dokan.sellerBadge,
            selector.admin.dokan.sellerBadge.sellerBadgeEdit(badgeName),
        );

        // badge condition box is visible
        await this.toBeVisible(
            selector.admin.dokan.sellerBadge.badgeDetails.badgeCondition
                .badgeConditionBox,
        );

        // badge event elements are visible
        const { badgeEvent, badgePublishedStatus, ...badgeEvents } =
            selector.admin.dokan.sellerBadge.badgeDetails.badgeEvents;
        await this.multipleElementVisible(badgeEvents);

        // badge photo elements are visible
        await this.multipleElementVisible(
            selector.admin.dokan.sellerBadge.badgeDetails.badgePhoto,
        );

        // badge status elements are visible
        const { create, ...badgeStatus } =
            selector.admin.dokan.sellerBadge.badgeDetails.badgeStatus;
    ...
    async createSellerBadge(badge: sellerBadge) {
        ...
        await this.clickAndWaitForResponse(
            data.subUrls.api.dokan.sellerBadgeEvent,
            selector.admin.dokan.sellerBadge.createBadge,
        );
        await this.click(
            selector.admin.dokan.sellerBadge.badgeDetails.badgeEvents
                .badgeEventDropdown,
        );
        const isPublished = await this.isVisible(
            selector.admin.dokan.sellerBadge.badgeDetails.badgeEvents.badgePublishedStatus(
                badge.badgeName,
            ),
        );
        if (isPublished) {
        ...
        await this.click(
            selector.admin.dokan.sellerBadge.badgeDetails.badgeEvents.badgeEvent(
                badge.badgeName,
            ),
        );
        await this.clearAndType(
            selector.admin.dokan.sellerBadge.badgeDetails.badgeEvents.badgeName,
            badge.badgeName,
        );

        const isLevelExists = await this.isVisible(
            selector.admin.dokan.sellerBadge.badgeDetails.badgeCondition
                .startingLevelValue,
        );
        if (isLevelExists) {
            ...
            if (badge.badgeName === 'Trending Product') {
                await this.selectByValue(
                    ...
                    badge.trendingProductPeriod,
                );
                await this.clearAndType(
                    selector.admin.dokan.sellerBadge.badgeDetails.badgeCondition
                        .trendingProductTopBestSellingProduct,
                    badge.trendingProductTopBestSellingProduct,
                );
            }
            if (badge.badgeName === 'Verified Seller') {
                ...
                const methods: string[] = Object.values(
                    badge.verifiedSellerMethod,
                );
                for (let i = 1; i <= methods.length; i++) {
                    await this.selectByValue(
                        selector.admin.dokan.sellerBadge.badgeDetails.badgeCondition.verifiedSellerMethod1(
                            i,
                        ),
                        methods[i - 1] as string,
                    );
                    if (i === methods.length) {
                        continue;
                    }
                    await this.click(
                        selector.admin.dokan.sellerBadge.badgeDetails
                            .badgeCondition.addBadgeLevel,
                    );
                }
            }
        }
        ...
        await this.clickAndWaitForResponse(
            data.subUrls.api.dokan.sellerBadge,
            selector.admin.dokan.sellerBadge.badgeDetails.badgeStatus.create,
        );
        await this.click(
            selector.admin.dokan.sellerBadge.badgeDetails
                .badgeAddedSuccessfully,
        );
    }
    ...
    async editSellerBadge(badge: sellerBadge) {
        ...
        await this.hover(
            selector.admin.dokan.sellerBadge.sellerBadgeRow(badge.badgeName),
        );
        await this.clickAndWaitForResponse(
            data.subUrls.api.dokan.sellerBadge,
            selector.admin.dokan.sellerBadge.sellerBadgeEdit(badge.badgeName),
        );
        ...
        if (isLevelExists) {
            ...
            const maxLevel = await this.countLocator(
                selector.admin.dokan.sellerBadge.badgeDetails.badgeCondition
                    .badgeLevel,
            );
            ...
            if (badge.badgeName === 'Verified Seller') {
                ...
                for (let i = 1; i < maxLevel; i++) {
                    await this.click(
                        selector.admin.dokan.sellerBadge.badgeDetails
                            .badgeCondition.removeBadgeLevel,
                    );
                }
        ...
        await this.click(
            selector.admin.dokan.sellerBadge.badgeDetails.badgeStatus.update,
        );
        await this.clickAndWaitForResponse(
            data.subUrls.api.dokan.sellerBadge,
            selector.admin.dokan.sellerBadge.badgeDetails.confirmBadgeUpdate,
        );
        await this.click(
            selector.admin.dokan.sellerBadge.badgeDetails
                .badgeAddedSuccessfully,
    ...
    async previewSellerBadge(badgeName: string) {
        ...
        const badgeLevel = await this.getElementText(
            selector.admin.dokan.sellerBadge.sellerBadgeLevel(badgeName),
        );
        ...
        await this.clickAndWaitForResponse(
            data.subUrls.api.dokan.sellerBadge,
            selector.admin.dokan.sellerBadge.sellerBadgePreview(badgeName),
        );

        // badge preview modal is visible
        await this.toBeVisible(
            selector.admin.dokan.sellerBadge.previewBadgeDetails.modal,
        );

        // badge preview header elements are visible
        await this.multipleElementVisible(
            selector.admin.dokan.sellerBadge.previewBadgeDetails.modalHeader,
        );

        await this.toHaveCount(
            selector.admin.dokan.sellerBadge.previewBadgeDetails.levelBox,
            Number(badgeLevel),
        );
    ...
    async filterVendorsByBadge(badgeName: string) {
        ...
        await this.clickIfVisible(
            selector.admin.dokan.vendors.filters.clearFilter,
        );
        await this.selectByLabel(
            selector.admin.dokan.vendors.filters.filterByBadges,
            badgeName,
        );
    ...
    async sellerBadgeVendors(badgeName: string) {
        ...
        await this.clickAndWaitForResponse(
            data.subUrls.api.dokan.sellerBadge,
            selector.admin.dokan.sellerBadge.sellerBadgeVendors(badgeName),
        );
    ...
    async sellerBadgeAcquiredByVendor(vendorName: string) {
        ...
        await this.clickAndWaitForResponse(
            data.subUrls.api.dokan.sellerBadge,
            selector.admin.dokan.vendors.vendorViewDetails(vendorName),
        );
        await this.toBeVisible(
    ...
    async updateSellerBadge(badgeName: string, status: string) {
        ...
        switch (status) {
            case 'publish':
                await this.click(
                    selector.admin.dokan.sellerBadge.sellerBadgePublish(
                        badgeName,
                    ),
                );
                break;
            ...
            case 'draft':
                await this.click(
                    selector.admin.dokan.sellerBadge.sellerBadgeDraft(
                        badgeName,
                    ),
                );
                break;
            ...
            case 'delete':
                await this.click(
                    selector.admin.dokan.sellerBadge.sellerBadgeDelete(
                        badgeName,
                    ),
                );
                break;
        ...
        await this.clickAndWaitForResponse(
            data.subUrls.api.dokan.sellerBadge,
            selector.admin.dokan.sellerBadge.confirmAction,
        );
    ...
    async sellerBadgeBulkAction(action: string, badgeName?: string) {
        ...
        await this.click(
            selector.admin.dokan.sellerBadge.bulkActions.selectAll,
        );
        await this.selectByValue(
            selector.admin.dokan.sellerBadge.bulkActions.selectAction,
            action,
        );
        await this.click(
            selector.admin.dokan.sellerBadge.bulkActions.applyAction,
        );
        await this.clickAndWaitForResponse(
            data.subUrls.api.dokan.sellerBadge,
            selector.admin.dokan.sellerBadge.confirmAction,
    ...
    async sellerBadgeCongratsPopup() {
        ...
        const congratsModalIsVisible = await this.isVisible(
            selector.vendor.vBadges.congratsModal.sellerBadgeModal,
        );
        if (congratsModalIsVisible) {
            //  seller badge congrats modal elements are visible
            await this.multipleElementVisible(
                selector.vendor.vBadges.congratsModal,
            );

            await this.clickIfVisible(
                selector.vendor.vBadges.congratsModal.closeModal,
            );
        } else {
    ...
    async vendorSearchSellerBadge(badgeName: string) {
        ...
        await this.toBeVisible(
            selector.vendor.vBadges.sellerBadgeCell(badgeName),
        );
    }
    ...
    async filterSellerBadges(option: string) {
        ...
        const count = (
            await this.getElementText(
                selector.vendor.vBadges.numberOfBadgesFound,
            )
        )?.split(' ')[0];
        expect(Number(count)).toBeGreaterThan(0);
    }
