...
export class MyOrdersPage extends CustomerPage {
    ...
    async myOrdersRenderProperly() {
        ...
        const noOrders = await this.isVisible(
        ...
        if (noOrders) {
            ...
            await this.toBeVisible(
                selector.customer.cMyOrders.recentOrdersText,
            );

            // my orders table elements are visible
            await this.multipleElementVisible(
                selector.customer.cMyOrders.table,
            );
        }
    }
    ...
    async viewOrderDetails(orderId: string) {
        ...
        await this.multipleElementVisible(
            selector.customer.cOrderDetails.orderDetails,
        );

        // customer details are visible
        await this.multipleElementVisible(
            selector.customer.cOrderDetails.customerDetails,
        );

        DOKAN_PRO &&
            (await this.toBeVisible(
                selector.customer.cOrderDetails.getSupport,
            ));
    }
    ...
    async viewOrderNote(orderNumber: string, orderNote: string) {
        ...
        await this.toBeVisible(
            selector.customer.cOrderDetails.orderUpdates.orderNote(orderNote),
        );
    }
    ...
    async payPendingOrder(orderId: string, paymentMethod = 'bank') {
        ...
        await this.clickAndWaitForResponse(
            data.subUrls.frontend.orderPay,
            selector.customer.cMyOrders.orderPay(orderId),
        );
    ...
    async cancelPendingOrder(orderId: string) {
        await this.goIfNotThere(data.subUrls.frontend.myOrders);
        await this.clickAndWaitForResponse(
            data.subUrls.frontend.orderCancel,
            selector.customer.cMyOrders.orderCancel(orderId),
            302,
        );
        await this.toContainText(
            selector.customer.cWooSelector.wooCommerceInfo,
    ...
    async orderAgain(orderId: string, paymentMethod = 'bank') {
        ...
        await this.clickAndAcceptAndWaitForResponse(
            data.subUrls.frontend.orderAgain,
            selector.customer.cOrderDetails.orderAgain,
            302,
        );
        await this.toContainText(
            selector.customer.cWooSelector.wooCommerceSuccessMessage,
            'The cart has been filled with the items from your previous order.',
        );
