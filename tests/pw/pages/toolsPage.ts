...
export class ToolsPage extends AdminPage {
    ...
    async dokanPageInstallation() {
        ...
        await this.hasClass(
            selector.admin.dokan.tools.pageInstallation.allPagesCreated,
            'button-disabled',
        );
    ...
    async checkForDuplicateOrders() {
        ...
        await this.clickAndWaitForResponse(
            data.subUrls.ajax,
            selector.admin.dokan.tools.checkForDuplicateOrders.checkOrders,
        );
    }
    ...
    async regenerateVariableProductVariationsAuthorIds() {
        ...
        await this.clickAndWaitForResponse(
            data.subUrls.ajax,
            selector.admin.dokan.tools
                .regenerateVariableProductVariationsAuthorIds.regenerate,
        );
    }
    ...
    async clearDummyData() {
        await this.goIfNotThere(data.subUrls.backend.dokan.tools);
        await this.clickAndWaitForResponse(
            data.subUrls.api.dokan.dummyData,
            selector.admin.dokan.tools.importDummyData.import,
        );
        await this.click(selector.admin.dokan.dummyData.clearDummyData);
        await this.clickAndWaitForResponse(
            data.subUrls.api.dokan.dummyData,
            selector.admin.dokan.dummyData.confirmClearDummyData,
        );
    }
    ...
    async testDistanceMatrixApi(address: tools['distanceMatrixApi']) {
        ...
        await this.clearAndType(
            selector.admin.dokan.tools.testDistanceMatrixApi.address1,
            address.address3,
        );
        await this.clearAndType(
            selector.admin.dokan.tools.testDistanceMatrixApi.address2,
            address.address4,
        );
        await this.click(
            selector.admin.dokan.tools.testDistanceMatrixApi.getDistance,
        );
        await this.toContainText(
            selector.admin.dokan.tools.testDistanceMatrixApi.enabledSuccess,
