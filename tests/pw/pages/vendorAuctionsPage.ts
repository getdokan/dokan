...
export class AuctionsPage extends VendorPage {
    ...
    async adminAddAuctionProduct(product: product['auction']) {
        ...
        await this.selectByValue(
            selector.admin.products.product.itemCondition,
            product.itemCondition,
        );
        await this.selectByValue(
            selector.admin.products.product.auctionType,
            product.auctionType,
        );
        await this.clearAndType(
            selector.admin.products.product.startPrice,
            product.regularPrice(),
        );
        await this.clearAndType(
            selector.admin.products.product.bidIncrement,
            product.bidIncrement(),
        );
        await this.clearAndType(
            selector.admin.products.product.reservedPrice,
            product.reservedPrice(),
        );
        await this.clearAndType(
            selector.admin.products.product.buyItNowPrice,
            product.buyItNowPrice(),
        );
        await this.clearAndType(
            selector.admin.products.product.auctionDatesFrom,
            product.startDate,
        );
        await this.clearAndType(
            selector.admin.products.product.auctionDatesTo,
            product.endDate,
        );
    ...
    async updateAuctionProductFields(product: product['auction']) {
        await this.clearAndType(
            selector.vendor.vAuction.auction.productName,
            product.name,
        );
        // await this.addProductCategory(product.category);
        await this.selectByValue(
            selector.vendor.vAuction.auction.itemCondition,
            product.itemCondition,
        );
        await this.selectByValue(
            selector.vendor.vAuction.auction.auctionType,
            product.auctionType,
        );
        await this.clearAndType(
            selector.vendor.vAuction.auction.startPrice,
            product.regularPrice(),
        );
        await this.clearAndType(
            selector.vendor.vAuction.auction.bidIncrement,
            product.bidIncrement(),
        );
        await this.clearAndType(
            selector.vendor.vAuction.auction.reservedPrice,
            product.reservedPrice(),
        );
        await this.clearAndType(
            selector.vendor.vAuction.auction.buyItNowPrice,
            product.buyItNowPrice(),
        );
        await this.removeAttribute(
            selector.vendor.vAuction.auction.auctionStartDate,
            'readonly',
        );
        await this.removeAttribute(
            selector.vendor.vAuction.auction.auctionEndDate,
            'readonly',
        );
        await this.clearAndType(
            selector.vendor.vAuction.auction.auctionStartDate,
    ...
    async addAuctionProduct(product: product['auction']) {
        ...
        await this.clickAndWaitForResponseAndLoadState(
            data.subUrls.frontend.vDashboard.productAuction,
            selector.vendor.vAuction.auction.addAuctionProduct,
            302,
        );
        await this.toContainText(
    ...
    async editAuctionProduct(product: product['auction']) {
        ...
        await this.clickAndWaitForLoadState(
            selector.vendor.vAuction.edit(product.name),
        );
        await this.updateAuctionProductFields(product);
        await this.clickAndWaitForResponseAndLoadState(
            data.subUrls.frontend.vDashboard.auction,
            selector.vendor.vAuction.auction.updateAuctionProduct,
            302,
        );
        await this.toContainText(
            selector.vendor.product.updatedSuccessMessage,
    ...
    async viewAuctionProduct(productName: string) {
        ...
        await this.clickAndWaitForLoadState(
            selector.vendor.vAuction.view(productName),
        );

        // auction product elements are visible
        const { bidQuantity, bidButton, ...viewAuction } =
            selector.vendor.vAuction.viewAuction;
    ...
    async cantBidOwnProduct(productName: string) {
        ...
        await this.toBeVisible(
            selector.vendor.vAuction.viewAuction.noBidOption,
        );
    }
    ...
    async searchAuctionProduct(productName: string) {
        ...
        await this.clickAndWaitForResponseAndLoadState(
            data.subUrls.frontend.vDashboard.auction,
            selector.vendor.vAuction.filters.filter,
        );
        await this.toBeVisible(
            selector.vendor.vAuction.productCell(productName),
        );
    }
    ...
    async deleteAuctionProduct(productName: string) {
        ...
        await this.hover(selector.vendor.vAuction.productCell(productName));
        await this.click(
            selector.vendor.vAuction.permanentlyDelete(productName),
        );
        await this.clickAndWaitForResponseAndLoadState(
            data.subUrls.frontend.vDashboard.auction,
            selector.vendor.vAuction.confirmDelete,
        );
        await this.toContainText(
            selector.vendor.vAuction.dokanSuccessMessage,
    ...
    async vendorAuctionActivityRenderProperly() {
        await this.goIfNotThere(
            data.subUrls.frontend.vDashboard.auctionActivity,
        );

        // auction activity text is visible
        await this.toBeVisible(
            selector.vendor.vAuction.actionActivity.actionActivityText,
        );

        // back to auctions is visible
        await this.toBeVisible(
            selector.vendor.vAuction.actionActivity.backToActions,
        );

        // filter elements are visible
        const { filterByDate, ...filters } =
            selector.vendor.vAuction.actionActivity.filters;
        await this.multipleElementVisible(filters);
        await this.toBeVisible(filterByDate.dateRangeInput);

        // search elements are visible
        await this.multipleElementVisible(
            selector.vendor.vAuction.actionActivity.search,
        );

        // auction activity table elements are visible
        await this.multipleElementVisible(
            selector.vendor.vAuction.actionActivity.table,
        );
    }
    ...
    async filterAuctionActivity(inputValue: date['dateRange']) {
        ...
        await this.setAttributeValue(
            selector.vendor.vAuction.actionActivity.filters.filterByDate
                .dateRangeInput,
            'value',
        ...
        await this.clickAndWaitForResponseAndLoadState(
            data.subUrls.frontend.vDashboard.auctionActivity,
            selector.vendor.vAuction.actionActivity.filters.filter,
        );
        await this.notToHaveCount(
            selector.vendor.vAuction.actionActivity.numOfRowsFound,
            0,
        );
    }
    ...
    async searchAuctionActivity(input: string) {
        ...
        await this.clearAndType(
            selector.vendor.vAuction.actionActivity.search.searchInput,
            input,
        );
        await this.clickAndWaitForResponseAndLoadState(
            data.subUrls.frontend.vDashboard.auctionActivity,
            selector.vendor.vAuction.actionActivity.search.search,
        );
        await this.notToHaveCount(
            selector.vendor.vAuction.actionActivity.numOfRowsFound,
            0,
