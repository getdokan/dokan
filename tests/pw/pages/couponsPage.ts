...
export class CouponsPage extends AdminPage {
    ...
    async addMarketplaceCoupon(coupon: coupon) {
        ...
        await this.page.waitForSelector('#publish:not([disabled])');
        await this.toBeEnabled(couponsAdmin.addNewCoupon.publish);
        
        // Click publish and wait for success message (more reliable than waiting for specific response)
        await this.click(couponsAdmin.addNewCoupon.publish);
