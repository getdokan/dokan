...
export class ShopPage extends CustomerPage {
    ...
    async shopRenderProperly() {
        ...
        await this.notToHaveCount(
            selector.customer.cShop.productCard.productDetailsLink,
            0,
        );
        await this.notToHaveCount(
        ...
        await this.notToHaveCount(
            selector.customer.cShop.productCard.productPrice,
            0,
        );
        await this.notToHaveCount(
    ...
    async sortProducts(sortBy: string) {
        ...
        await this.selectByValueAndWaitForResponse(
            data.subUrls.frontend.shop,
            selector.customer.cShop.sort,
            sortBy,
        );
    }
        ...
        switch (filterBy) {
            case 'by-location':
                await this.typeAndWaitForResponse(
                    data.subUrls.gmap,
                    selector.customer.cShop.filters.location,
                    value,
                );
            ...
            case 'by-category':
                await this.selectByValue(
                    selector.customer.cShop.filters.selectCategory,
                    value,
                );
                break;
    ...
    async productOnMap(productName?: string) {
        ...
        const storePinIsVisible = await this.isVisible(
            selector.customer.cShop.map.productOnMap.productPin,
        );
        if (storePinIsVisible) {
            ...
            await this.toBeVisible(
                selector.customer.cShop.map.productOnMap.productPopup,
            );
        } else {
            await this.click(
                selector.customer.cShop.map.productOnMap.productCluster,
            );
            await this.toBeVisible(
                selector.customer.cShop.map.productOnMap.productListPopup,
            );
            await this.click(
                selector.customer.cShop.map.productOnMap.closePopup,
            );
        }
        productName &&
            (await this.toBeVisible(
                selector.customer.cShop.map.productOnMap.productOnList(
                    productName,
                ),
            ));
    }
