...
export class TaxPage extends AdminPage {
    ...
    async enableTax(enable = true) {
        ...
        enable
            ...
            : await this.uncheck(
                  selector.admin.wooCommerce.settings.enableTaxes,
              );
        await this.click(
    ...
    async addStandardTaxRate(tax: tax) {
        ...
        const taxIsVisible = await this.isVisible(
            selector.admin.wooCommerce.settings.taxRate,
        );
        if (!taxIsVisible) {
        ...
        await this.click(
            selector.admin.wooCommerce.settings.taxRateSaveChanges,
        );

        const newTaxRate = await this.getElementValue(
            selector.admin.wooCommerce.settings.taxRate,
        );
