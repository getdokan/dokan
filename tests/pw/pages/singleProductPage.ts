...
export class SingleProductPage extends CustomerPage {
    ...
    async singleProductRenderProperly(productName: string) {
        ...
        await this.multipleElementVisible(
            selector.customer.cSingleProduct.description,
        );
        ...
        await this.toBeVisible(
            selector.customer.cSingleProduct.reviews.ratings,
        );
        await this.toBeVisible(
            selector.customer.cSingleProduct.reviews.reviewMessage,
        );
        await this.toBeVisible(
            selector.customer.cSingleProduct.reviews.submitReview,
        );

        // vendor info elements are visible
        await this.click(selector.customer.cSingleProduct.menus.vendorInfo);
        await this.multipleElementVisible(
            selector.customer.cSingleProduct.vendorInfo,
        );

        // more products elements are visible
        await this.click(selector.customer.cSingleProduct.menus.moreProducts);
        await this.toBeVisible(
            selector.customer.cSingleProduct.moreProducts.moreProductsDiv,
        );
        await this.notToHaveCount(
            selector.customer.cSingleProduct.moreProducts.product,
            0,
        );

        // related products elements are visible
        await this.multipleElementVisible(
            selector.customer.cSingleProduct.relatedProducts,
        );

        if (DOKAN_PRO) {
            // get support is visible
            await this.toBeVisible(
                selector.customer.cSingleProduct.getSupport.getSupport,
            );
            ...
            await this.multipleElementVisible(
                selector.customer.cSingleProduct.vendorHighlightedInfo,
            );
            ...
            await this.multipleElementVisible(
                selector.customer.cSingleProduct.location,
            );
            ...
            await this.click(
                selector.customer.cSingleProduct.menus.productEnquiry,
            );
            const { submitEnquirySuccessMessage, guest, ...productEnquiry } =
                selector.customer.cSingleProduct.productEnquiry;
    ...
    async reviewProduct(
        ...
        await this.click(selector.customer.cSingleProduct.menus.reviews);
        await this.click(
            selector.customer.cSingleProduct.reviews.rating(review.rating),
        );
        ...
        await this.clickAndWaitForResponseAndLoadState(
            data.subUrls.frontend.productReview,
            selector.customer.cSingleProduct.reviews.submitReview,
            302,
        );
        await this.toContainText(
            selector.customer.cSingleProduct.reviews.submittedReview(
                reviewMessage,
            ),
            reviewMessage,
        );
    ...
    async productWarrantyPolicy(productName: string) {
        ...
        await this.click(selector.customer.cSingleProduct.menus.warrantyPolicy);
        await this.multipleElementVisible(
            selector.customer.cSingleProduct.warrantyPolicy,
        );
    }
