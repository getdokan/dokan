...
export class VendorSettingsPage extends VendorPage {
    ...
    async vendorStoreSettingsRenderProperly() {
        ...
        await this.toBeVisible(
            selector.vendor.vStoreSettings.storeProductsPerPage,
        );
        await this.toBeVisible(selector.vendor.vStoreSettings.phoneNo);
        DOKAN_PRO &&
            (await this.toBeVisible(
                selector.vendor.vStoreSettings.multipleLocation,
            ));
        ...
        const {
            ...
            deleteSaveLocation,
            ...address
        } = selector.vendor.vStoreSettings.address;
        ...
        DOKAN_PRO &&
            (await this.multipleElementVisible(
                selector.vendor.vStoreSettings.companyInfo,
            ));
        ...
        DOKAN_PRO &&
            (await this.toBeVisible(
                selector.vendor.vStoreSettings.biographyIframe,
            ));
        ...
        await this.toBeVisible(
            selector.vendor.vStoreSettings.updateSettingsTop,
        );
    ...
    async vendorShipstationSettingsRenderProperly() {
        await this.goIfNotThere(
            data.subUrls.frontend.vDashboard.settingsShipstation,
        );

        // shipstation text is visible
        await this.toBeVisible(
            selector.vendor.vShipStationSettings.shipStationText,
        );
        ...
        await this.toBeVisible(
            selector.vendor.vShipStationSettings.authenticationKey,
        );

        // export order statuses is visible
        await this.toBeVisible(
            selector.vendor.vShipStationSettings.exportOrderStatusesInput,
        );

        // Shipped Order Status is visible
        await this.toBeVisible(
            selector.vendor.vShipStationSettings.shippedOrderStatusDropdown,
        );

        // save changes is visible
        await this.toBeVisible(
            selector.vendor.vShipStationSettings.saveChanges,
        );
    }
    ...
    async vendorSocialProfileSettingsRenderProperly() {
        await this.goIfNotThere(
            data.subUrls.frontend.vDashboard.settingsSocialProfile,
        );

        // social profile text is visible
        await this.toBeVisible(
            selector.vendor.vSocialProfileSettings.socialProfileText,
        );

        // visit store link is visible
        await this.toBeVisible(
            selector.vendor.vSocialProfileSettings.visitStore,
        );

        // social platform elements are visible
        await this.multipleElementVisible(
            selector.vendor.vSocialProfileSettings.platforms,
        );

        // update settings is visible
        await this.toBeVisible(
            selector.vendor.vSocialProfileSettings.updateSettings,
        );
    }
    ...
    async vendorRmaSettingsRenderProperly() {
        ...
        await this.toBeVisible(
            selector.vendor.vRmaSettings.returnAndWarrantyText,
        );
    ...
    async vendorStoreSeoSettingsRenderProperly() {
        ...
        await this.toBeVisible(
            selector.vendor.vStoreSeoSettings.metaDescription,
        );
        ...
        await this.multipleElementVisible(
            selector.vendor.vStoreSeoSettings.facebook,
        );

        // store seo twitter elements are visible
        await this.multipleElementVisible(
            selector.vendor.vStoreSeoSettings.twitter,
        );
    ...
    async updateStoreMapViaSettingsSave() {
        ...
        await this.clickAndWaitForResponseAndLoadState(
            data.subUrls.ajax,
            selector.vendor.vStoreSettings.updateSettings,
        );
        await this.toContainText(
            selector.vendor.vStoreSettings.updateSettingsSuccessMessage,
    ...
    async basicInfoSettings(vendorInfo: vendor['vendorInfo']): Promise<void> {
        ...
        await this.clearAndType(
            selector.vendor.vStoreSettings.storeName,
            vendorInfo.storeName,
        );
        await this.clearAndType(
            selector.vendor.vStoreSettings.storeProductsPerPage,
            vendorInfo.productsPerPage,
        );
        await this.clearAndType(
            selector.vendor.vStoreSettings.phoneNo,
    ...
    async setStoreAddress(vendorInfo: vendor['vendorInfo']): Promise<void> {
        await this.clearAndType(
            selector.vendor.vStoreSettings.address.street,
            vendorInfo.street1,
        );
        await this.clearAndType(
            selector.vendor.vStoreSettings.address.street2,
            vendorInfo.street2,
        );
        await this.clearAndType(
            selector.vendor.vStoreSettings.address.city,
            vendorInfo.city,
        );
        await this.clearAndType(
            selector.vendor.vStoreSettings.address.postOrZipCode,
            vendorInfo.zipCode,
        );
        await this.selectByValue(
            selector.vendor.vStoreSettings.address.country,
            vendorInfo.countrySelectValue,
        );
        await this.selectByValue(
            selector.vendor.vStoreSettings.address.state,
            vendorInfo.stateSelectValue,
        );
    }
    ...
    async setCompanyInfo(vendorInfo: vendor['vendorInfo']): Promise<void> {
        await this.clearAndType(
            selector.vendor.vStoreSettings.companyInfo.companyName,
            vendorInfo.companyName,
        );
        await this.clearAndType(
            selector.vendor.vStoreSettings.companyInfo.companyId,
            vendorInfo.companyId,
        );
        await this.clearAndType(
            selector.vendor.vStoreSettings.companyInfo.vatOrTaxNumber,
            vendorInfo.vatNumber,
        );
        await this.clearAndType(
            selector.vendor.vStoreSettings.companyInfo.nameOfBank,
            vendorInfo.bankName,
        );
        await this.clearAndType(
            selector.vendor.vStoreSettings.companyInfo.bankIban,
            vendorInfo.bankIban,
        );
    }
    ...
    async mapSettings(mapLocation: string): Promise<void> {
        const geoLocationEnabled = await this.isVisible(
            selector.vendor.vStoreSettings.map,
        );
    ...
    async termsAndConditionsSettings(
        ...
        if (tocEnabled) {
            ...
            await this.typeFrameSelector(
                selector.vendor.vStoreSettings.termsAndConditionsIframe,
                selector.vendor.vStoreSettings.termsAndConditionsHtmlBody,
                termsAndConditions,
            );
        }
    }
    ...
    async openingClosingTimeSettings(
        openingClosingTime: vendor['vendorInfo']['openingClosingTime'],
    ): Promise<void> {
        ...
        if (openCloseTimeEnabled) {
            ...
            for (const day of openingClosingTime.days) {
                if (DOKAN_PRO) {
                    await this.enableSwitcherDeliveryTime(
                        selector.vendor.vStoreSettings.openingClosingTimeSwitch(
                            day,
                        ),
                    );
                    await this.setAttributeValue(
                        selector.vendor.vStoreSettings.openingTime(day),
                        'value',
                        openingClosingTime.openingTime,
                    );
                    await this.setAttributeValue(
                        selector.vendor.vStoreSettings.openingTimeHiddenInput(
                            day,
                        ),
                        'value',
                        openingClosingTime.openingTime,
                    ...
                    await this.setAttributeValue(
                        selector.vendor.vStoreSettings.closingTime(day),
                        'value',
                        openingClosingTime.closingTime,
                    );
                    await this.setAttributeValue(
                        selector.vendor.vStoreSettings.closingTimeHiddenInput(
                            day,
                        ),
                        'value',
                        openingClosingTime.closingTime,
                    ...
                    // lite
                    await this.selectByValue(
                        selector.vendor.vStoreSettings.lite.openingClosingTimeEnable(
                            day,
                        ),
                        openingClosingTime.statusLite,
                    );
                    await this.clearAndType(
                        selector.vendor.vStoreSettings.lite.openingTimeInput(
                            day,
                        ),
                        openingClosingTime.openingTime,
                    );
                    await this.clearAndType(
                        selector.vendor.vStoreSettings.lite.closingTimeInput(
                            day,
                        ),
                        openingClosingTime.closingTime,
                    );
            ...
            await this.clearAndType(
                selector.vendor.vStoreSettings.storeOpenNotice,
                openingClosingTime.storeOpenNotice,
            );
            await this.clearAndType(
                selector.vendor.vStoreSettings.storeCloseNotice,
                openingClosingTime.storeCloseNotice,
            );
        }
    }
    ...
    async vacationSettings(
        vacation: vendor['vendorInfo']['vacation'],
    ): Promise<void> {
        const vacationModeEnabled = await this.isVisible(
            selector.vendor.vStoreSettings.goToVacation,
        );
        if (vacationModeEnabled) {
            await this.check(selector.vendor.vStoreSettings.goToVacation);
            await this.selectByValue(
                selector.vendor.vStoreSettings.closingStyle,
                vacation.closingStyle,
            );
            ...
            switch (vacation.closingStyle) {
                ...
                case 'instantly':
                    await this.clearAndType(
                        selector.vendor.vStoreSettings
                ...
                case 'datewise': {
                    ...
                    const vacationDayTo =
                        vacation.datewise.vacationDayTo(vacationDayFrom);
                    await this.setAttributeValue(
                        selector.vendor.vStoreSettings.vacationDateRange,
                        'value',
                        helpers.dateFormatFYJ(vacationDayFrom) +
                            ' - ' +
                            helpers.dateFormatFYJ(vacationDayTo),
                    );
                    await this.setAttributeValue(
                        selector.vendor.vStoreSettings.vacationDateRangeFrom,
                        'value',
                        vacationDayFrom,
                    );
                    await this.setAttributeValue(
                        selector.vendor.vStoreSettings.vacationDateRangeTo,
                        'value',
                        vacationDayTo,
                    );
                    await this.clearAndType(
                        ...
                        vacation.datewise.vacationMessage,
                    );
                    await this.clickAndWaitForResponse(
                        data.subUrls.ajax,
                        selector.vendor.vStoreSettings.saveVacationEdit,
                    );
                    break;
                }
    ...
    async deletePreviousDatewiseVacation() {
        await this.goIfNotThere(data.subUrls.frontend.vDashboard.settingsStore);
        const noVacationIsSetIsVisible = await this.isVisible(
            selector.vendor.vStoreSettings.noVacationIsSet,
        );
        if (!noVacationIsSetIsVisible) {
            await this.hover(selector.vendor.vStoreSettings.vacationRow);
            await this.click(
                selector.vendor.vStoreSettings.deleteSavedVacationSchedule,
            );
            await this.clickAndWaitForResponse(
                data.subUrls.ajax,
                selector.vendor.vStoreSettings
                    .confirmDeleteSavedVacationSchedule,
            );
        }
    ...
    async catalogModeSettings(): Promise<void> {
        const catalogModeEnabled = await this.isVisible(
            selector.vendor.vStoreSettings.removeAddToCartButton,
        );
        if (catalogModeEnabled) {
            ...
            await this.checkIfVisible(
                selector.vendor.vStoreSettings.enableRequestQuoteSupport,
            );
        }
    }
    ...
    async discountSettings(
        amountDiscount: vendor['vendorInfo']['amountDiscount'],
    ): Promise<void> {
        const discountEnabled = await this.isVisible(
            selector.vendor.vStoreSettings.enableStoreWideDiscount,
        );
        if (discountEnabled) {
            await this.check(
                selector.vendor.vStoreSettings.enableStoreWideDiscount,
            ...
            await this.clearAndType(
                selector.vendor.vStoreSettings.minimumOrderAmount,
                amountDiscount.minimumOrderAmount,
            );
            await this.clearAndType(
                selector.vendor.vStoreSettings.percentage,
                amountDiscount.discountPercentage,
            );
        }
    }
    ...
    async biographySettings(biography: string): Promise<void> {
        await this.typeFrameSelector(
            selector.vendor.vStoreSettings.biographyIframe,
            selector.vendor.vStoreSettings.biographyHtmlBody,
            biography,
        );
    }
    ...
    async storeSupportSettings(supportButtonText: string): Promise<void> {
        const storeSupportEnabled = await this.isVisible(
            selector.vendor.vStoreSettings.removeAddToCartButton,
        );
        if (storeSupportEnabled) {
            await this.check(
                selector.vendor.vStoreSettings.showSupportButtonInStore,
            );
            await this.check(
                selector.vendor.vStoreSettings.showSupportButtonInSingleProduct,
            );
            await this.clearAndType(
                selector.vendor.vStoreSettings.supportButtonText,
                supportButtonText,
            );
        }
    }
    ...
    async minMaxSettings(
        minMax: vendor['vendorInfo']['minMax'],
    ): Promise<void> {
        const minMaxEnabled = await this.isVisible(
            selector.vendor.vStoreSettings.enableMinMaxQuantities,
        );
        if (minMaxEnabled) {
            // min max quantities
            await this.check(
            ...
            await this.clearAndType(
                selector.vendor.vStoreSettings
                    .minimumProductQuantityToPlaceAnOrder,
                minMax.minimumProductQuantity,
            );
            await this.clearAndType(
                selector.vendor.vStoreSettings
                    .maximumProductQuantityToPlaceAnOrder,
                minMax.maximumProductQuantity,
            );

            // min max amount
            await this.check(selector.vendor.vStoreSettings.enableMinMaxAmount);
            await this.clearAndType(
                selector.vendor.vStoreSettings.minimumAmountToPlaceAnOrder,
                minMax.minimumAmount,
            );
            await this.clearAndType(
                selector.vendor.vStoreSettings.maximumAmountToPlaceAnOrder,
                minMax.maximumAmount,
            );
            await this.click(selector.vendor.vStoreSettings.clear);
            await this.click(selector.vendor.vStoreSettings.selectAll);
            const multipleCategory = await this.isVisible(
                selector.vendor.vStoreSettings.selectCategorySearch,
            );
            if (multipleCategory) {
                await this.select2ByTextMultiSelector(
                    selector.vendor.vStoreSettings.selectCategorySearch,
                    selector.vendor.vStoreSettings.selectCategorySearchedResult,
                    minMax.category,
                );
            } else {
                await this.selectByLabel(
                    selector.vendor.vStoreSettings.selectCategory,
                    minMax.category,
                );
            }
        }
    ...
    async setShipStation(shipStation: vendor['shipStation']): Promise<void> {
        ...
        const allStatus = await this.getMultipleElementTexts(
            selector.vendor.vShipStationSettings.selectedStatus,
        );
        ...
        if (!statusIsSelected) {
            ...
            await this.toContainText(
                selector.vendor.vShipStationSettings.result,
                shipStation.status,
            );
            await this.press(data.key.enter);
        }
        ...
        await this.toContainText(
            selector.vendor.vShipStationSettings.saveSuccessMessage,
    ...
    async setSocialProfile(urls: vendor['socialProfileUrls']): Promise<void> {
        ...
        await this.clearAndType(
            selector.vendor.vSocialProfileSettings.platforms.facebook,
            urls.facebook,
        );
        await this.clearAndType(
            selector.vendor.vSocialProfileSettings.platforms.twitter,
            urls.twitter,
        );
        await this.clearAndType(
            selector.vendor.vSocialProfileSettings.platforms.pinterest,
            urls.pinterest,
        );
        await this.clearAndType(
            selector.vendor.vSocialProfileSettings.platforms.linkedin,
            urls.linkedin,
        );
        await this.clearAndType(
            selector.vendor.vSocialProfileSettings.platforms.youtube,
            urls.youtube,
        );
        await this.clearAndType(
            selector.vendor.vSocialProfileSettings.platforms.instagram,
            urls.instagram,
        );
        await this.clearAndType(
            selector.vendor.vSocialProfileSettings.platforms.flickr,
            urls.flickr,
        );
        await this.keyPressOnLocator(
            selector.vendor.vSocialProfileSettings.updateSettings,
            data.key.enter,
        );
        await this.toContainText(
            selector.vendor.vSocialProfileSettings.updateSettingsSuccessMessage,
            urls.saveSuccessMessage,
        );
    }
    ...
    async setRmaSettings(rma: vendor['rma']): Promise<void> {
        ...
        await this.selectByValue(
            selector.vendor.vRmaSettings.length,
            rma.rmaLength,
        );
        // todo: add addon
        if (rma.rmaLength === 'limited') {
            await this.clearAndType(
                selector.vendor.vRmaSettings.lengthValue,
                rma.lengthValue,
            );
            await this.selectByValue(
                selector.vendor.vRmaSettings.lengthDuration,
                rma.lengthDuration,
            );
        }
        // check if refund reason exists
        const refundReasonIsVisible = await this.isVisible(
            selector.vendor.vRmaSettings.refundReasonsFirst,
        );
        if (refundReasonIsVisible) {
            await this.checkMultiple(
                selector.vendor.vRmaSettings.refundReasons,
            );
        }
        await this.typeFrameSelector(
            selector.vendor.vRmaSettings.refundPolicyIframe,
            selector.vendor.vRmaSettings.refundPolicyHtmlBody,
            rma.refundPolicyHtmlBody,
        );
        await this.clickAndWaitForResponseAndLoadState(
            data.subUrls.frontend.vDashboard.settingsRma,
            selector.vendor.vRmaSettings.saveChanges,
            302,
        );
        await this.toContainText(
            selector.vendor.vRmaSettings.updateSettingsSuccessMessage,
    ...
    async setStoreSeo(seo: vendor['seo']): Promise<void> {
        ...
        await this.clearAndType(
            selector.vendor.vStoreSeoSettings.seoTitle,
            seo.seoTitle,
        );
        await this.clearAndType(
            selector.vendor.vStoreSeoSettings.metaDescription,
        ...
        await this.clearAndType(
            selector.vendor.vStoreSeoSettings.metaKeywords,
            seo.metaKeywords,
        );

        await this.clearAndType(
            selector.vendor.vStoreSeoSettings.facebook.facebookTitle,
            seo.facebookTitle,
        );
        await this.clearAndType(
            selector.vendor.vStoreSeoSettings.facebook.facebookDescription,
            seo.facebookDescription,
        );

        await this.clearAndType(
            selector.vendor.vStoreSeoSettings.twitter.twitterTitle,
            seo.twitterTitle,
        );
        await this.clearAndType(
            selector.vendor.vStoreSeoSettings.twitter.twitterDescription,
            seo.twitterDescription,
        );

        await this.clickAndWaitForResponseAndLoadState(
            data.subUrls.ajax,
            selector.vendor.vStoreSeoSettings.saveChanges,
        );
        await this.toContainText(
            selector.vendor.vStoreSeoSettings.updateSettingsSuccessMessage,
            'Your changes has been updated!',
        );
    }
}
