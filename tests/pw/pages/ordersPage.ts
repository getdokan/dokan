...
export class OrdersPage extends VendorPage {
    ...
    async vendorOrdersRenderProperly(): Promise<void> {
        ...
        const { filterByCustomer, filterByDate, ...filters } =
            selector.vendor.orders.filters;
        await this.toBeVisible(
            selector.vendor.orders.filters.filterByCustomer.dropDown,
        );
        await this.toBeVisible(
        ...
        switch (type) {
            case 'all':
                ...
                await this.clickAndWaitForDownload(
                    selector.vendor.orders.export.exportAll,
                );
                break;
    ...
    async filterOrders(
        ...
        switch (filterBy) {
            case 'by-customer':
                ...
                await this.typeAndWaitForResponse(
                    data.subUrls.ajax,
                    selector.vendor.orders.filters.filterByCustomer.input,
                    inputValue as string,
                );
                await this.click(
                    selector.vendor.orders.filters.filterByCustomer
                        .searchedResult,
                );
                break;
            ...
            case 'by-date':
                ...
                await this.setAttributeValue(
                    selector.vendor.orders.filters.filterByDate.startDateInput,
                    'value',
                    inputValue.startDate,
                ); // todo: resolve this
                await this.setAttributeValue(
                    selector.vendor.orders.filters.filterByDate.endDateInput,
                    'value',
                    inputValue.endDate,
                );
                break;
        ...
        await this.clickAndWaitForLoadState(
            selector.vendor.orders.filters.filter,
        );
    ...
    async viewOrderDetails(orderNumber: string): Promise<void> {
        ...
        await this.click(
            selector.vendor.orders.trackingDetails.addTrackingNumber,
        );

        // tracking detail elements are visible
        const { addTrackingNumber, ...trackingDetails } =
            selector.vendor.orders.trackingDetails;
        ...
        if (DOKAN_PRO) {
            ...
            const {
                createNewShipment,
                shipmentOrderItem,
                shipmentOrderItemQty,
                ...shipment
            } = selector.vendor.orders.shipment;
        ...
        switch (status) {
            case 'processing':
                await this.clickAndAcceptAndWaitForResponse(
                    data.subUrls.ajax,
                    selector.vendor.orders.processing(orderNumber),
                    302,
                );
            ...
            case 'complete':
                await this.clickAndAcceptAndWaitForResponse(
                    data.subUrls.ajax,
                    selector.vendor.orders.complete(orderNumber),
                    302,
                );
                await this.notToBeVisible(
                    selector.vendor.orders.complete(orderNumber),
    ...
    async updateOrderStatus(
        ...
        await this.selectByValue(
            selector.vendor.orders.status.orderStatus,
            status,
        );
        await this.clickAndAcceptAndWaitForResponse(
            data.subUrls.ajax,
            selector.vendor.orders.status.updateOrderStatus,
        );
        const currentStatus = await this.getElementText(
    ...
    async addOrderNote(
        ...
        await this.goToOrderDetails(orderNumber);
        await this.clearAndType(
            selector.vendor.orders.orderNote.orderNoteInput,
            orderNote.note,
        );
        await this.selectByLabel(
            selector.vendor.orders.orderNote.orderNoteType,
            orderNote.noteType,
        );
        await this.clickAndAcceptAndWaitForResponse(
            data.subUrls.ajax,
            selector.vendor.orders.orderNote.addNote,
        );
    }
    ...
    async addTrackingDetails(
        ...
        await this.clearAndType(
            selector.vendor.orders.trackingDetails.shippingProvider,
            orderTrackingDetails.shippingProvider,
        );
        await this.clearAndType(
            selector.vendor.orders.trackingDetails.trackingNumber,
            orderTrackingDetails.trackingNumber,
        );
        await this.setAttributeValue(
            selector.vendor.orders.trackingDetails.dateShipped,
            'value',
            helpers.dateFormatFYJ(orderTrackingDetails.dateShipped),
        );
        await this.clickAndAcceptAndWaitForResponse(
            data.subUrls.ajax,
            selector.vendor.orders.trackingDetails.addTrackingDetails,
        );
    }
    ...
    async addShipment(
        ...
        await this.click(selector.vendor.orders.shipment.createNewShipment);
        await this.click(
            selector.vendor.orders.shipment.shipmentOrderItem(
                shipmentDetails.shipmentOrderItem,
            ),
        );
        await this.clearAndType(
            selector.vendor.orders.shipment.shipmentOrderItemQty(
                shipmentDetails.shipmentOrderItem,
            ),
            shipmentDetails.shipmentOrderItemQty,
        );
        await this.selectByValue(
            selector.vendor.orders.shipment.shippingStatus,
            shipmentDetails.shippingStatus,
        );
        await this.selectByValue(
            selector.vendor.orders.shipment.shippingProvider,
            shipmentDetails.shippingProvider,
        );
        await this.setAttributeValue(
            selector.vendor.orders.shipment.dateShipped,
            'value',
            helpers.dateFormatFYJ(shipmentDetails.dateShipped),
        );
        await this.clearAndType(
            selector.vendor.orders.shipment.trackingNumber,
            shipmentDetails.shippingProvider,
        );
        await this.clearAndType(
            selector.vendor.orders.shipment.comments,
            shipmentDetails.trackingNumber,
        );
        await this.click(selector.vendor.orders.shipment.notifyCustomer);
        await this.clickAndAcceptAndWaitForResponse(
            data.subUrls.ajax,
            selector.vendor.orders.shipment.createShipment,
        );
    ...
    async addDownloadableProduct(
        orderNumber: string,
        downloadableProductName: string,
    ): Promise<void> {
        await this.goToOrderDetails(orderNumber);
        await this.clearAndType(
            selector.vendor.orders.downloadableProductPermission
                .downloadableProductInput,
            downloadableProductName,
        );
        await this.press(data.key.enter);
        await this.clickAndAcceptAndWaitForResponseAndLoadState(
            data.subUrls.ajax,
            selector.vendor.orders.downloadableProductPermission.grantAccess,
        ); // todo:  need to fix
    }

    // add Downloadable Product
    async removeDownloadableProduct(
        orderNumber: string,
        downloadableProductName: string,
    ): Promise<void> {
        await this.addDownloadableProduct(orderNumber, downloadableProductName); // todo: do it via api
        await this.click(
            selector.vendor.orders.downloadableProductPermission.revokeAccess,
        );
        await this.clickAndAcceptAndWaitForResponse(
            data.subUrls.ajax,
            selector.vendor.orders.downloadableProductPermission.confirmAction,
        );
    }
    ...
    async orderBulkAction(action: string, orderNumber?: string): Promise<void> {
        ...
        switch (action) {
            case 'onhold':
                await this.selectByValue(
                    selector.vendor.orders.bulkActions.selectAction,
                    'wc-on-hold',
                );
                break;
        ...
        await this.clickAndAcceptAndWaitForResponseAndLoadState(
            data.subUrls.frontend.vDashboard.orders,
            selector.vendor.orders.bulkActions.applyAction,
        );
    }
}
