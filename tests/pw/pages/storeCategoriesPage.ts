...
export class StoreCategoriesPage extends AdminPage {
    ...
    async adminStoreCategoryRenderProperly() {
        ...
        await this.multipleElementVisible(
            selector.admin.dokan.vendors.storeCategory.addNewCategory,
        );

        // search category input is visible
        await this.toBeVisible(
            selector.admin.dokan.vendors.storeCategory.search,
        );

        // store category table elements are visible
        await this.multipleElementVisible(
            selector.admin.dokan.vendors.storeCategory.table,
        );
    }
    ...
    async addStoreCategory(storeCategory: storeCategory) {
        ...
        await this.clearAndType(
            selector.admin.dokan.vendors.storeCategory.addNewCategory.name,
            storeCategory.name,
        );
        await this.clearAndType(
            selector.admin.dokan.vendors.storeCategory.addNewCategory
        ...
        await this.clickAndWaitForResponse(
            data.subUrls.api.dokan.storeCategories,
            selector.admin.dokan.vendors.storeCategory.addNewCategory
                .addNewCategory,
        );
    }
    ...
    async searchStoreCategory(categoryName: string) {
        ...
        await this.toBeVisible(
            selector.admin.dokan.vendors.storeCategory.storeCategoryCell(
                categoryName,
            ),
        );
    }
    ...
    async editStoreCategory(storeCategory: storeCategory) {
        ...
        await this.clickAndWaitForResponse(
            data.subUrls.api.dokan.storeCategories,
            selector.admin.dokan.vendors.storeCategory.storeCategoryEdit(
                storeCategory.name,
            ),
        );
        await this.clearAndType(
            selector.admin.dokan.vendors.storeCategory.editCategory.name,
            storeCategory.name,
        );
        await this.clearAndType(
            selector.admin.dokan.vendors.storeCategory.editCategory.description,
            storeCategory.description,
        );
        await this.clickAndWaitForResponse(
            data.subUrls.api.dokan.storeCategories,
            selector.admin.dokan.vendors.storeCategory.editCategory.update,
        );
    }
    ...
    async updateStoreCategory(categoryName: string, action: string) {
        ...
        switch (action) {
            case 'set-default':
                await this.clickAndWaitForResponse(
                    data.subUrls.api.dokan.storeCategories,
                    selector.admin.dokan.vendors.storeCategory.storeCategorySetDefault(
                        categoryName,
                    ),
                );
                break;
            ...
            case 'delete':
                await this.clickAndAcceptAndWaitForResponse(
                    data.subUrls.api.dokan.storeCategories,
                    selector.admin.dokan.vendors.storeCategory.storeCategoryDelete(
                        categoryName,
                    ),
                );
                break;
    ...
    async vendorUpdateStoreCategory(category: string) {
        ...
        const isSingleCategory = await this.isVisible(
        ...
        await this.toContainText(
            selector.vendor.vStoreSettings.storeCategories.result,
            category,
        );
