...
export class ReverseWithdrawsPage extends CustomerPage {
    ...
    async adminReverseWithdrawRenderProperly() {
        ...
        await this.toBeVisible(
            selector.admin.dokan.reverseWithdraw.reverseWithdrawText,
        );

        // add new reverse withdrawal is visible
        await this.toBeVisible(
            selector.admin.dokan.reverseWithdraw.addNewReverseWithdrawal,
        );

        // fact cards elements are visible
        await this.multipleElementVisible(
            selector.admin.dokan.reverseWithdraw.reverseWithdrawFactCards,
        );

        // filter elements are visible
        const { filterInput, clearFilter, filteredResult, ...filters } =
            selector.admin.dokan.reverseWithdraw.filters;
        await this.multipleElementVisible(filters);

        // reverse withdraw table elements are visible
        await this.multipleElementVisible(
            selector.admin.dokan.reverseWithdraw.table,
        );
    }
    ...
    async filterReverseWithdraws(vendorName: string) {
        ...
        await this.clickIfVisible(
            selector.admin.dokan.reverseWithdraw.filters.clearFilter,
        );

        await this.click(
            selector.admin.dokan.reverseWithdraw.filters.filterByStore,
        );
        await this.typeAndWaitForResponse(
            data.subUrls.api.dokan.reverseWithdraws,
            selector.admin.dokan.reverseWithdraw.filters.filterInput,
            vendorName,
        );
        await this.clickAndWaitForResponseAndLoadState(
            data.subUrls.api.dokan.reverseWithdraws,
            selector.admin.dokan.reverseWithdraw.filters.filteredResult(
                vendorName,
            ),
        );
        await this.toBeVisible(
            selector.admin.dokan.reverseWithdraw.reverseWithdrawCell(
                vendorName,
            ),
        );
    }
    ...
    async addReverseWithdrawal(reverseWithdrawal: reverseWithdraw) {
        ...
        await this.click(
            selector.admin.dokan.reverseWithdraw.addReverseWithdrawal
        ...
        await this.typeAndWaitForResponse(
            ...
            selector.admin.dokan.reverseWithdraw.addReverseWithdrawal
                .selectVendorInput,
            reverseWithdrawal.store,
        );
        await this.click(
            selector.admin.dokan.reverseWithdraw.addReverseWithdrawal.selectOption(
                reverseWithdrawal.store,
            ),
        );
        ...
        await this.click(
            selector.admin.dokan.reverseWithdraw.addReverseWithdrawal.transactionType(
        ...
        await this.click(
            selector.admin.dokan.reverseWithdraw.addReverseWithdrawal.withdrawalBalanceType(
                reverseWithdrawal.withdrawalBalanceType,
            ),
        );
        ...
        await this.clearAndType(
            ...
            reverseWithdrawal.amount,
        );
        await this.clearAndType(
            selector.admin.dokan.reverseWithdraw.addReverseWithdrawal.note,
            reverseWithdrawal.note,
        );

        await this.clickAndWaitForResponseAndLoadState(
            data.subUrls.api.dokan.reverseWithdraws,
            selector.admin.dokan.reverseWithdraw.addReverseWithdrawal.save,
        );
    }
    ...
    async vendorReverseWithdrawalRenderProperly() {
        await this.goIfNotThere(
            data.subUrls.frontend.vDashboard.reverseWithdrawal,
        );

        // reverse withdrawal text is visible
        await this.toBeVisible(
            selector.vendor.vReverseWithdrawal.reverseWithdrawalText,
        );

        // reverse balance section elements are visible
        await this.multipleElementVisible(
            selector.vendor.vReverseWithdrawal.reverseBalanceSection,
        );
        ...
        await this.toBeVisible(
            selector.vendor.vReverseWithdrawal.filters.filter,
        );

        // reverse withdrawal table elements are visible
        await this.multipleElementVisible(
            selector.vendor.vReverseWithdrawal.table,
        );

        // opening balance row is visible
        await this.toBeVisible(
            selector.vendor.vReverseWithdrawal.openingBalanceRow,
        );
    }
    ...
    async vendorViewReverseWithdrawalNotice() {
        ...
        await this.toBeVisible(
            selector.vendor.vReverseWithdrawal.reverseWithdrawalNotice
                .noticeText,
        );
    ...
    async vendorViewReverseWithdrawalAnnouncement() {
        ...
        await this.clickAndWaitForLoadState(
            selector.vendor.vAnnouncement.firstAnnouncementLink(
                'You have a reverse withdrawal balance of',
            ),
        );
    ...
    async vendorFilterReverseWithdrawals(inputValue: date['dateRange']) {
        ...
        await this.setAttributeValue(
            selector.vendor.vReverseWithdrawal.filters.startDateInput,
            'value',
            inputValue.startDate,
        );
        await this.setAttributeValue(
            selector.vendor.vReverseWithdrawal.filters.endDateInput,
            'value',
            inputValue.endDate,
        );
        await this.clickAndWaitForLoadState(
        ...
        await this.notToHaveCount(
            selector.vendor.vReverseWithdrawal.numberOfRowsFound,
            3,
        );
    }
    ...
    async vendorPayReversePayBalance() {
        ...
        await this.clickAndWaitForResponse(
            data.subUrls.ajax,
            selector.vendor.vReverseWithdrawal.payNow,
        );
        await this.clickAndWaitForResponseAndLoadState(
            data.subUrls.frontend.checkout,
            selector.vendor.vReverseWithdrawal.confirmAction,
        );
