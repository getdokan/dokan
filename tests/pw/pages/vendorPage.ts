...
export class VendorPage extends BasePage {
    ...
    async vendorRegister(
        ...
        setupWizardData: vendorSetupWizard,
    ): Promise<void> {
        const username =
            vendorInfo.firstName() + vendorInfo.lastName().replace("'", '');
        ...
        await this.clearAndType(
            selector.vendor.vRegistration.regEmail,
            username + data.vendor.vendorInfo.emailDomain,
        );
        await this.clearAndType(
            selector.vendor.vRegistration.regPassword,
            vendorInfo.password,
        );
        await this.focusAndClick(selector.vendor.vRegistration.regVendor);
        await this.waitForVisibleLocator(
            selector.vendor.vRegistration.firstName,
        );
        await this.clearAndType(
            selector.vendor.vRegistration.firstName,
            username,
        ...
        await this.clearAndType(
            selector.vendor.vRegistration.lastName,
            vendorInfo.lastName(),
        );
        await this.clearAndType(
            selector.vendor.vRegistration.shopName,
            vendorInfo.shopName(),
        );
        await this.click(selector.vendor.vRegistration.shopUrl);

        // fill address if enabled on registration
        const addressInputIsVisible = await this.isVisible(
            selector.vendor.vRegistration.street1,
        );
        if (addressInputIsVisible) {
            ...
            await this.clearAndType(
                selector.vendor.vRegistration.street2,
                vendorInfo.street2,
            );
            await this.clearAndType(
                selector.vendor.vRegistration.city,
                vendorInfo.city,
            );
            await this.clearAndType(
                selector.vendor.vRegistration.zipCode,
                vendorInfo.zipCode,
            );
            await this.selectByValue(
                selector.vendor.vRegistration.country,
                vendorInfo.countrySelectValue,
            );
            await this.selectByValue(
                selector.vendor.vRegistration.state,
                vendorInfo.stateSelectValue,
            );
        }
        if (DOKAN_PRO) {
            await this.clearAndType(
                selector.vendor.vRegistration.companyName,
                vendorInfo.companyName,
            );
            await this.clearAndType(
                selector.vendor.vRegistration.companyId,
                vendorInfo.companyId,
            );
            await this.clearAndType(
                selector.vendor.vRegistration.vatNumber,
                vendorInfo.vatNumber,
            );
            await this.clearAndType(
                selector.vendor.vRegistration.bankName,
                vendorInfo.bankName,
            );
            await this.clearAndType(
                selector.vendor.vRegistration.bankIban,
                vendorInfo.bankIban,
            );
        }
        await this.clearAndType(
            selector.vendor.vRegistration.phone,
        ...
        await this.clickAndWaitForResponseAndLoadState(
            data.subUrls.frontend.vDashboard.setupWizard,
            selector.vendor.vRegistration.register,
        );
        ...
        subscriptionPackIsVisible &&
            (await this.customer.placeOrder('bank', false, true, false));
    ...
    async vendorSetupWizard(setupWizardData: vendorSetupWizard): Promise<void> {
        ...
        if (setupWizardData.choice) {
            ...
            await this.clearAndType(
                selector.vendor.vSetup.storeProductsPerPage,
                setupWizardData.storeProductsPerPage,
            );
            await this.clearAndType(
                selector.vendor.vSetup.street1,
                setupWizardData.street1,
            );
            await this.clearAndType(
                selector.vendor.vSetup.street2,
                setupWizardData.street2,
            );
            await this.clearAndType(
                selector.vendor.vSetup.city,
                setupWizardData.city,
            );
            await this.clearAndType(
                selector.vendor.vSetup.zipCode,
                setupWizardData.zipCode,
            );
            await this.click(selector.vendor.vSetup.country);
            await this.type(
                selector.vendor.vSetup.countryInput,
                setupWizardData.country,
            );
            await this.toContainText(
                selector.vendor.vSetup.highlightedResult,
                setupWizardData.country,
            );
            await this.press(data.key.enter);
            await this.click(selector.vendor.vSetup.state);
            await this.type(
                selector.vendor.vSetup.stateInput,
            ...
            if (storeCategoriesEnabled) {
                const allStoreCategories = await this.getMultipleElementTexts(
                    selector.vendor.vSetup.selectedStoreCategories,
                );
                const categoryIsSelected = allStoreCategories.includes(
                    '×' + setupWizardData.storeCategory,
                );
                if (!categoryIsSelected) {
                    await this.click(selector.vendor.vSetup.storeCategories);
                    await this.type(
                        selector.vendor.vSetup.storeCategoriesInput,
                        setupWizardData.storeCategory,
                    );
                    await this.toContainText(
                        selector.vendor.vSetup.highlightedResult,
            ...
            const geoLocationEnabled = await this.isVisible(
                selector.vendor.vSetup.map,
            );
            if (geoLocationEnabled) {
                await this.typeAndWaitForResponse(
                    data.subUrls.gmap,
                    selector.vendor.vSetup.map,
                    setupWizardData.mapLocation,
                );
            ...
            // paypal
            await this.clearAndType(
                selector.vendor.vSetup.paypal,
                setupWizardData.paypal(),
            );
            // bank transfer
            await this.clearAndType(
                selector.vendor.vSetup.bankAccountName,
                setupWizardData.bankAccountName,
            );
            await this.selectByValue(
                selector.vendor.vSetup.bankAccountType,
                setupWizardData.bankAccountType,
            );
            await this.clearAndType(
                selector.vendor.vSetup.bankAccountNumber,
                setupWizardData.bankAccountNumber,
            );
            await this.clearAndType(
                selector.vendor.vSetup.bankRoutingNumber,
                setupWizardData.bankRoutingNumber,
            );
            await this.clearAndType(
                selector.vendor.vSetup.bankName,
                setupWizardData.bankName,
            );
            await this.clearAndType(
                selector.vendor.vSetup.bankAddress,
                setupWizardData.bankAddress,
            );
            await this.clearAndType(
                selector.vendor.vSetup.bankIban,
                setupWizardData.bankIban,
            );
            await this.clearAndType(
                selector.vendor.vSetup.bankSwiftCode,
                setupWizardData.bankSwiftCode,
            );
            await this.check(selector.vendor.vSetup.declaration);
            // custom method
            await this.typeIfVisible(
                selector.vendor.vSetup.customPayment,
                setupWizardData.customPayment,
            );
            // skrill
            await this.typeIfVisible(
                selector.vendor.vSetup.skrill,
            ...
            await this.clickAndWaitForResponseAndLoadState(
                ...
                selector.vendor.vSetup.goToStoreDashboard,
            );
        } else {
            await this.clickAndWaitForResponseAndLoadState(
                data.subUrls.frontend.vDashboard.dashboard,
    ...
    async vendorAccountDetailsRenderProperly() {
        await this.goIfNotThere(
            data.subUrls.frontend.vDashboard.editAccountVendor,
        );

        const { saveSuccessMessage, ...accountDetails } =
            selector.vendor.vAccountDetails;
    ...
    async addVendorDetails(vendor: vendor): Promise<void> {
        ...
        await this.clearAndType(
            selector.vendor.vAccountDetails.firstName,
            vendor.username,
        );
        await this.clearAndType(
            selector.vendor.vAccountDetails.lastName,
            vendor.lastname,
        );
        await this.clearAndType(
            selector.vendor.vAccountDetails.email,
            vendor.username + vendor.vendorInfo.emailDomain,
        );
        // await this.updatePassword(vendor.vendorInfo.password, vendor.vendorInfo.password1);
        await this.clickAndWaitForResponseAndLoadState(
            data.subUrls.frontend.vDashboard.editAccountVendor,
            selector.vendor.vAccountDetails.saveChanges,
            302,
        );
        await expect(
            this.page.getByText(
                selector.vendor.vAccountDetails.saveSuccessMessage,
            ),
        ).toBeVisible();
        await this.toContainText(
            selector.customer.cWooSelector.wooCommerceSuccessMessage,
            data.vendor.vendorInfo.account.updateSuccessMessage,
        );
    ...
    async updatePassword(
        ...
        await this.type(
            selector.vendor.vAccountDetails.currentPassword,
            currentPassword,
        );
        await this.type(
            selector.vendor.vAccountDetails.NewPassword,
            newPassword,
        );
        await this.type(
            selector.vendor.vAccountDetails.confirmNewPassword,
            newPassword,
        );
        if (saveChanges) {
    ...
    async getTotalVendorEarning(): Promise<number> {
        ...
        return helpers.price(
            (await this.getElementText(
                selector.vendor.vDashboard.atAGlance.earningValue,
            )) as string,
        );
    }
    ...
    async getOrderDetails(orderNumber: string): Promise<object> {
        ...
        orderDetails.vendorEarning = helpers.price(
            (await this.getElementText(
                selector.vendor.orders.vendorEarningTable(orderNumber),
            )) as string,
        );
        await this.clickAndWaitForLoadState(
            selector.vendor.orders.view(orderNumber),
        ...
        const refundedOrderTotalIsVisible = await this.isVisible(
            selector.vendor.orders.orderDetails.orderTotalAfterRefund,
        );
        if (refundedOrderTotalIsVisible) {
            orderDetails.orderTotalBeforeRefund = helpers.price(
                (await this.getElementText(
                    selector.vendor.orders.orderDetails.orderTotalBeforeRefund,
                )) as string,
            );
            ...
            orderDetails.orderTotal = helpers.price(
                (await this.getElementText(
                    selector.vendor.orders.orderDetails.orderTotal,
                )) as string,
            );
        }
        ...
        orderDetails.orderStatus = (
            (await this.getElementText(
                selector.vendor.orders.status.currentOrderStatus,
            )) as string
        ).replace('-', ' ');

        const orderDate = (
            (await this.getElementText(
                selector.vendor.orders.orderDetails.orderDate,
            )) as string
        )
            ?.split(':')[1]
            ?.trim() as string;
        orderDetails.orderDate = orderDate?.substring(
            0,
            orderDate.indexOf(',', orderDate.indexOf(',') + 1),
        );
        ...
        const shippingMethodIsVisible = await this.isVisible(
            selector.vendor.orders.orderDetails.shippingMethod,
        );
        if (shippingMethodIsVisible) {
            orderDetails.shippingCost = helpers.price(
                (await this.getElementText(
                    selector.vendor.orders.orderDetails.shippingCost,
                )) as string,
            );
        }

        const taxIsVisible = await this.isVisible(
            selector.vendor.orders.orderDetails.tax,
        );
        if (taxIsVisible) {
        ...
        const refundIsVisible = await this.isVisible(
            selector.vendor.orders.orderDetails.refunded,
        );
        if (refundIsVisible) {
            orderDetails.refunded = helpers.price(
                (await this.getElementText(
                    selector.vendor.orders.orderDetails.refunded,
                )) as string,
            );
    ...
    async visitStore(storeName: string) {
        ...
        await this.toHaveAttribute(
            selector.vendor.vDashboard.menus.visitStore,
            'target',
            '_blank',
        );
    ...
    async buyProductAdvertising(productName: string) {
        await this.searchProduct(productName);
        const advertisementStatus = await this.hasColor(
            selector.vendor.product.advertisementStatus(productName),
            'rgb(255, 99, 71)',
        );
        ...
        await this.clickAndWaitForResponse(
            data.subUrls.ajax,
            selector.vendor.product.buyAdvertisement(productName),
        );
        await this.clickAndWaitForResponse(
            data.subUrls.ajax,
