...
export class SingleStorePage extends CustomerPage {
    ...
    async singleStoreRenderProperly(storeName: string) {
        ...
        await this.notToHaveCount(
            selector.customer.cSingleStore.productCard.card,
            0,
        );
        await this.notToHaveCount(
            selector.customer.cSingleStore.productCard.productDetailsLink,
            0,
        );
        await this.notToHaveCount(
            selector.customer.cSingleStore.productCard.productTitle,
            0,
        );
        await this.notToHaveCount(
            selector.customer.cSingleStore.productCard.productPrice,
            0,
        );
        await this.notToHaveCount(
            selector.customer.cSingleStore.productCard.addToCart,
            0,
        );

        // store social icons are visible
        await this.multipleElementVisible(
            selector.customer.cSingleStore.storeSocialIcons,
        );
    }
    ...
    async singleStoreSortProducts(storeName: string, sortBy: string) {
        ...
        await this.selectByValueAndWaitForResponse(
            data.subUrls.frontend.vendorDetails(helpers.slugify(storeName)),
            selector.customer.cSingleStore.sortBy,
            sortBy,
        );
    }

    // search product on single store
    async singleStoreSearchProduct(
        storeName: string,
        productName: string,
    ): Promise<void> {
        await this.goIfNotThere(
        ...
        await this.clearAndType(
            selector.customer.cSingleStore.search.input,
            productName,
        );
        await this.clickAndWaitForResponse(
            data.subUrls.frontend.vendorDetails(helpers.slugify(storeName)),
            selector.customer.cSingleStore.search.button,
        );
    ...
    async storeOpenCloseTime(storeName: string): Promise<void> {
        ...
        await this.hover(
            selector.customer.cSingleStore.storeTime.storeTimeDropDown,
        );

        await this.toBeVisible(
            selector.customer.cSingleStore.storeTime.storeTimeDiv,
        );
        await this.toBeVisible(
            selector.customer.cSingleStore.storeTime.storeTimeHeading,
        );

        await this.toHaveCount(
            selector.customer.cSingleStore.storeTime.storeDays,
            7,
        );
        await this.toHaveCount(
            selector.customer.cSingleStore.storeTime.storeTimes,
            7,
        );
    }
    ...
    async storeTermsAndCondition(
        ...
        await this.clickAndWaitForResponse(
            data.subUrls.frontend.vendorDetails(helpers.slugify(storeName)),
            selector.customer.cSingleStore.storeTabs.toc,
        );
        await this.toContainText(
            selector.customer.cSingleStore.toc.tocContent,
            toc,
        );
    }
    ...
    async storeShare(storeName: string, site: string): Promise<void> {
        ...
        await this.toHaveAttribute(
            selector.customer.cSingleStore.sharePlatForms[
                site as keyof typeof selector.customer.cSingleStore.sharePlatForms
            ],
            'target',
            '_blank',
        );
