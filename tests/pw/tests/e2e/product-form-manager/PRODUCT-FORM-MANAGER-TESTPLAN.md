...
| F13 | **Both** `GET` and `POST /settings` share one `permission_callback` = `current_user_can('manage_woocommerce')`. Vendors/customers are forbidden on **read and write** (403); guests get 401. (Verified against the live REST API — the earlier "GET falls back to default auth" note was wrong.) | Permission negatives cover GET *and* POST for vendor/customer/guest. |
| F14 | **Vendor REST basic-auth works** in this env (`vendor1 / USER_PASSWORD` → 200). The earlier `VENDOR_AUTH_BROKEN` assumption was false — vendor-owned product creation and custom-field persistence are fully automatable via REST. | Persistence round-trips and vendor reflection run for real (no skips). |
| F15 | Custom field values persist **only on create** (`dokan_new_product_added`). A partial vendor REST update (`{fieldId: newValue}`) does **not** re-save the meta. Storefront output of the value is theme/cache-sensitive. | Automate create-persistence (PFM-S01/S08); leave update round-trip (S04) and storefront non-render (W04) as documented manual gaps. |
| F16 | The admin builder & vendor editor are **hash-routed SPAs**: `page.goto()` to the same fragment is a no-op (no reload), leaking React state across tests. | Navigation must force a fresh document load (route through `about:blank`). |
| F17 | `#dokan-form-field-{id}` is emitted only by `CustomField`-wrapped variants (select/textarea/editor/datetime/image/file/price/attributes…). Plain **text/number** custom fields render with no such wrapper. | Assert custom text/number fields by their rendered `.dokan-form-field-label`, not the id wrapper. |
| F18 | The REST sanitizer (`sanitize_schema_item`) has no `requireds` branch, so **per-product-type required overrides are silently dropped** on save (`labels`/`visibilities` persist). | Risk R5 — assert current behavior (requireds absent after save). |
