...
test.describe('Booking Product test', () => {
    ...
    test('admin can add booking product', { tag: ['@pro', '@a'] }, async () => {
        await admin.adminAddBookingProduct(data.product.booking);
    });

    // vendor

    test('vendor booking menu page is rendering properly', { tag: ['@pro', '@exp', '@v'] }, async () => {
        await vendor.vendorBookingRenderProperly();
    });

    test('vendor manage booking page is rendering properly', { tag: ['@pro', '@exp', '@v'] }, async () => {
        await vendor.vendorManageBookingRenderProperly();
    });

    test('vendor booking calendar page is rendering properly', { tag: ['@pro', '@exp', '@v'] }, async () => {
        await vendor.vendorBookingCalendarRenderProperly();
    });

    test('vendor manage booking resource page is rendering properly', { tag: ['@pro', '@exp', '@v'] }, async () => {
        await vendor.vendorManageResourcesRenderProperly();
    });

    test('vendor can add booking product', { tag: ['@pro', '@v'] }, async () => {
        await vendor.addBookingProduct({ ...data.product.booking, name: data.product.booking.productName() });
    });

    test('vendor can edit booking product', { tag: ['@pro', '@v'] }, async () => {
        await vendor.editBookingProduct({ ...data.product.booking, name: bookableProductName });
    });

    test('vendor can filter booking products by date', { tag: ['@pro', '@v'] }, async () => {
        await vendor.filterBookingProducts('by-date', '1');
    });

    test('vendor can filter booking products by category', { tag: ['@pro', '@v'] }, async () => {
        await vendor.filterBookingProducts('by-category', 'Uncategorized');
    });

    test('vendor can filter booking products by other', { tag: ['@pro', '@v'] }, async () => {
        await vendor.filterBookingProducts('by-other', 'featured');
    });

    test('vendor can view booking product', { tag: ['@pro', '@v'] }, async () => {
        await vendor.viewBookingProduct(bookableProductName);
    });

    test("vendor can't buy own booking product", { tag: ['@pro', '@v'] }, async () => {
        await vendor.cantBuyOwnBookingProduct(bookableProductName);
    });

    test('vendor can search booking product', { tag: ['@pro', '@v'] }, async () => {
        await vendor.searchBookingProduct(bookableProductName);
    });

    test('vendor can duplicate booking product', { tag: ['@pro', '@v'] }, async () => {
        await vendor.duplicateBookingProduct(bookableProductName);
    });

    test('vendor can permanently delete booking product', { tag: ['@pro', '@v'] }, async () => {
    ...
    test('vendor can add booking resource', { tag: ['@pro', '@v'] }, async () => {
        await vendor.addBookingResource(data.product.booking.resource.resourceName());
    });

    test('vendor can edit booking resource', { tag: ['@pro', '@v'] }, async () => {
    ...
    test('vendor can delete booking resource', { tag: ['@pro', '@v'] }, async () => {
        const bookingResourceName = data.product.booking.resource.resourceName();
        await vendor.addBookingResource(bookingResourceName); // todo: convert with woo api:fatal error exits on api
        await vendor.deleteBookingResource(bookingResourceName);
    });

    test('vendor can add booking for guest customer', { tag: ['@pro', '@v'] }, async () => {
        await vendor.addBooking(bookableProductName, data.bookings);
    });

    test.skip('vendor can add booking for existing customer', { tag: ['@pro', '@v'] }, async () => {
        await vendor.addBooking(bookableProductName, data.bookings, data.customer.username);
    });

    test.skip('customer can buy bookable product', { tag: ['@pro', '@c'] }, async () => {
