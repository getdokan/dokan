...
setup.describe('setup site & woocommerce & user settings', () => {
    ...
    setup('check active plugins @lite', async () => {
        ...
        const activePlugins = (
            await apiUtils.getAllPlugins({ status: 'active' })
        ).map((a: { plugin: string }) => a.plugin.split('/')[1]);
        DOKAN_PRO
            ? expect(activePlugins).toEqual(
                  expect.arrayContaining(data.plugin.plugins),
              )
            : expect(activePlugins).toEqual(
                  expect.arrayContaining(data.plugin.pluginsLite),
              );
    ...
    setup('check active dokan modules @pro', async () => {
        ...
        expect(activeModules).toEqual(
            expect.arrayContaining(data.modules.modules),
        );
    });

    setup('set wp settings @lite', async () => {
        const siteSettings = await apiUtils.setSiteSettings(
            payloads.siteSettings,
        );
        expect(siteSettings).toEqual(
            expect.objectContaining(payloads.siteSettings),
        );
    });
    ...
    setup('set wc settings @lite', async () => {
        await apiUtils.updateBatchWcSettingsOptions(
            'general',
            payloads.general,
        );
        await apiUtils.updateBatchWcSettingsOptions(
    ...
    setup('set shipping methods @lite', async () => {
        ...
        const allShippingZoneIds = (await apiUtils.getAllShippingZones()).map(
            (a: { id: string }) => a.id,
        );
        ...
        const [, zoneId] = await apiUtils.createShippingZone(
            payloads.createShippingZone,
        );
        await apiUtils.addShippingZoneLocation(
            zoneId,
            payloads.addShippingZoneLocation,
        );
        await apiUtils.addShippingZoneMethod(
            zoneId,
            payloads.addShippingMethodFlatRate,
        );
        ...
        if (DOKAN_PRO) {
            await apiUtils.addShippingZoneMethod(
                zoneId,
                payloads.addShippingMethodDokanTableRateShipping,
            );
            await apiUtils.addShippingZoneMethod(
                zoneId,
                payloads.addShippingMethodDokanDistanceRateShipping,
            );
            await apiUtils.addShippingZoneMethod(
                zoneId,
                payloads.addShippingMethodDokanVendorShipping,
            );
        }
    });
    ...
    setup('add categories and attributes @lite', async () => {
        ...
        const allCategoryIds = (await apiUtils.getAllCategories()).map(
            (a: { id: string }) => a.id,
        );
        await apiUtils.updateBatchCategories('delete', allCategoryIds);

        // delete previous attributes
        const allAttributeIds = (await apiUtils.getAllAttributes()).map(
            (a: { id: string }) => a.id,
        );
    ...
    setup('disable simple-auction ajax bid check @pro', async () => {
        ...
        const [, , status] = await apiUtils.getSinglePlugin(
            'wa',
            payloads.adminAuth,
        ); // todo: remove after ..
        status === 'active' &&
            (await dbUtils.updateWpOptionTable(
                'simple_auctions_live_check',
                'no',
            ));
    });
});
...
setup.describe('setup user settings', () => {
    ...
    setup('add vendor1 product @lite', async () => {
        ...
        await apiUtils.deleteAllProducts(
    ...
    setup('add vendor coupon @pro', async () => {
        // create store coupon
        const allProductIds = (
            await apiUtils.getAllProducts(payloads.vendorAuth)
        ).map((o: { id: string }) => o.id);
        await apiUtils.createCoupon(
            allProductIds,
            payloads.createCoupon1,
            payloads.vendorAuth,
        );
    });
...
setup.describe('setup dokan settings', () => {
    ...
    setup('set dokan general settings @lite', async () => {
        await dbUtils.setDokanSettings(
            dbData.dokan.optionName.general,
    ...
    setup('admin set dokan withdraw settings @lite', async () => {
        await dbUtils.setDokanSettings(
            dbData.dokan.optionName.withdraw,
            dbData.dokan.withdrawSettings,
        );
    });

    setup('admin set dokan reverse withdraw settings @lite', async () => {
        await dbUtils.setDokanSettings(
            dbData.dokan.optionName.reverseWithdraw,
            dbData.dokan.reverseWithdrawSettings,
        );
    });

    setup('admin set dokan page settings @lite', async () => {
        const [, pageId] = await apiUtils.createPage(
            payloads.tocPage,
            payloads.adminAuth,
        );
        const pageSettings = await dbUtils.getDokanSettings(
            dbData.dokan.optionName.page,
        );
    ...
    setup('admin set dokan appearance settings @lite', async () => {
        await dbUtils.setDokanSettings(
            dbData.dokan.optionName.appearance,
    ...
    setup('admin set dokan privacy policy settings @lite', async () => {
        await dbUtils.setDokanSettings(
            dbData.dokan.optionName.privacyPolicy,
            dbData.dokan.privacyPolicySettings,
        );
    });

    setup('admin set dokan color settings @pro', async () => {
        await dbUtils.setDokanSettings(
            dbData.dokan.optionName.colors,
            dbData.dokan.colorsSettings,
        );
    });

    setup('admin set dokan store support settings @pro', async () => {
        await dbUtils.setDokanSettings(
            dbData.dokan.optionName.storeSupport,
            dbData.dokan.storeSupportSettings,
        );
    });

    setup('admin set dokan shipping status settings @pro', async () => {
        await dbUtils.setDokanSettings(
            dbData.dokan.optionName.shippingStatus,
            dbData.dokan.shippingStatusSettings,
        );
    });

    setup('admin set dokan quote settings @pro', async () => {
        await dbUtils.setDokanSettings(
            dbData.dokan.optionName.quote,
            dbData.dokan.quoteSettings,
        );
    });

    setup('admin set dokan rma settings @pro', async () => {
        await dbUtils.setDokanSettings(
            dbData.dokan.optionName.rma,
            dbData.dokan.rmaSettings,
        );
    });

    setup('admin set dokan wholesale settings @pro', async () => {
        await dbUtils.setDokanSettings(
            dbData.dokan.optionName.wholesale,
    ...
    setup('admin set dokan eu compliance settings @pro', async () => {
        await dbUtils.setDokanSettings(
            dbData.dokan.optionName.euCompliance,
            dbData.dokan.euComplianceSettings,
        );
    });

    setup('admin set dokan delivery time settings @pro', async () => {
        await dbUtils.setDokanSettings(
            dbData.dokan.optionName.deliveryTime,
    ...
    setup('admin set dokan product advertising settings @pro', async () => {
        await dbUtils.setDokanSettings(
            dbData.dokan.optionName.productAdvertising,
            dbData.dokan.productAdvertisingSettings,
        );
    });

    setup('admin set dokan geolocation settings @pro', async () => {
        await dbUtils.setDokanSettings(
            dbData.dokan.optionName.geolocation,
            dbData.dokan.geolocationSettings,
        );
    });
    ...
    setup('admin set dokan spmv settings @pro', async () => {
        await dbUtils.setDokanSettings(
            dbData.dokan.optionName.spmv,
            dbData.dokan.spmvSettings,
        );
    });
...
setup.describe('setup dokan settings e2e', () => {
    ...
    setup(
        'recreate product advertisement payment product via settings save @pro',
        async () => {
    ...
    setup('product advertisement payment product exists @pro', async () => {
        const product = await apiUtils.checkProductExistence(
    ...
    setup(
        'recreate reverse withdrawal payment product via settings save @lite',
        async () => {
