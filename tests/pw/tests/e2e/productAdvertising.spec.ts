...
test.describe('Product Advertising test', () => {
    ...
    test('dokan product advertising menu page is rendering properly', { tag: ['@pro', '@exp', '@a'] }, async () => {
        await admin.adminProductAdvertisingRenderProperly();
    });

    test('admin can add product advertisement', { tag: ['@pro', '@a'] }, async () => {
        await admin.addNewProductAdvertisement({ ...data.productAdvertisement, advertisedProduct: productName });
    });

    test('admin can search advertised product', { tag: ['@pro', '@a'] }, async () => {
        await admin.searchAdvertisedProduct(advertisedProduct);
    });

    test('admin can filter advertised product by stores', { tag: ['@pro', '@a'] }, async () => {
        await admin.filterAdvertisedProduct(data.productAdvertisement.filter.byStore, 'by-store');
    });

    test('admin can filter advertised product by creation process', { tag: ['@pro', '@a'] }, async () => {
        await admin.filterAdvertisedProduct(data.productAdvertisement.filter.createVia.admin, 'by-creation');
    });

    test('admin can expire advertised product', { tag: ['@pro', '@a'] }, async () => {
        await admin.updateAdvertisedProduct(productName, 'expire');
    });

    test('admin can delete advertised product', { tag: ['@pro', '@a'] }, async () => {
    ...
    test.skip('admin can perform product advertising bulk action', { tag: ['@pro', '@a'] }, async () => {
    ...
    test.skip('vendor can buy product advertising', { tag: ['@pro', '@v'] }, async () => {
    ...
    test.skip('vendor can buy booking product advertising', { tag: ['@pro', '@v'] }, async () => {
    ...
    test.skip('vendor can buy auction product advertising', { tag: ['@pro', '@v'] }, async () => {
