...
test.describe('Payments test', () => {
    ...
    test.skip('admin can add basic payment methods', { tag: ['@lite', '@a'] }, async () => {
        await admin.setupBasicPaymentMethods(data.payment);
    });

    test.skip('admin can add strip payment method', { tag: ['@pro', '@a'] }, async () => {
        await admin.setupStripeConnect(data.payment);
    });

    test.skip('admin can add paypal marketplace payment method', { tag: ['@pro', '@a'] }, async () => {
        await admin.setupPaypalMarketPlace(data.payment);
    });

    test.skip('admin can add mangopay payment method', { tag: ['@pro', '@a'] }, async () => {
        await admin.setupMangoPay(data.payment);
    });

    test.skip('admin can add razorpay payment method', { tag: ['@pro', '@a'] }, async () => {
        await admin.setupRazorpay(data.payment);
    });

    test.skip('admin can add strip express payment method', { tag: ['@pro', '@a'] }, async () => {
        await admin.setupStripeExpress(data.payment);
    });

    //vendor

    test('vendor payment menu is rendering properly', { tag: ['@lite', '@exp', '@v'] }, async () => {
        await vendor.vendorPaymentSettingsRenderProperly();
    });

    test('vendor can add paypal payment method', { tag: ['@lite', '@v'] }, async () => {
        await vendor.setBasicPayment({ ...data.vendor.payment, methodName: 'paypal' });
    });

    test('vendor can add bank payment method', { tag: ['@lite', '@v'] }, async () => {
        await vendor.setBankTransfer(data.vendor.payment);
    });

    test('vendor can add skrill payment method', { tag: ['@pro', '@v'] }, async () => {
        await vendor.setBasicPayment({ ...data.vendor.payment, methodName: 'skrill' });
    });

    test('vendor can add custom payment method', { tag: ['@pro', '@v'] }, async () => {
        await vendor.setBasicPayment({ ...data.vendor.payment, methodName: 'custom' });
    });

    test('vendor can disconnect paypal payment method', { tag: ['@lite', '@v'] }, async () => {
    ...
    test('vendor can disconnect bank payment method', { tag: ['@lite', '@v'] }, async () => {
        await vendor.disconnectBasicPayment({ ...data.vendor.payment, methodName: 'bank' });
        // reset
        await apiUtils.setStoreSettings(payloads.defaultStoreSettings, payloads.vendorAuth);
    });

    test('vendor can disconnect skrill payment method', { tag: ['@pro', '@v'] }, async () => {
        await vendor.disconnectBasicPayment({ ...data.vendor.payment, methodName: 'skrill' });
    });

    test('vendor can disconnect custom payment method', { tag: ['@pro', '@v'] }, async () => {
