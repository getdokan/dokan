...
test.describe.skip('Settings test', () => {
    ...
    test('admin can set vendor setup wizard logo & message (general settings)', { tag: ['@lite', '@admin'] }, async () => {
        ...
        await dbUtils.setOptionValue(dbData.dokan.optionName.general, {
    ...
    test('admin can disable vendor setup wizard (general settings)', { tag: ['@lite', '@guest'] }, async () => {
        await dbUtils.setOptionValue(dbData.dokan.optionName.general, { ...dbData.dokan.generalSettings, disable_welcome_wizard: 'on' });
    ...
    test('admin can set store terms and conditions (general settings)', { tag: ['@lite', '@vendor'] }, async () => {
        await dbUtils.setOptionValue(dbData.dokan.optionName.general, { ...dbData.dokan.generalSettings, seller_enable_terms_and_conditions: 'on' });
        await vendor.setStoreTermsAndConditions('on');

        await dbUtils.setOptionValue(dbData.dokan.optionName.general, { ...dbData.dokan.generalSettings, seller_enable_terms_and_conditions: 'off' });
    ...
    test('admin can set store products per page (general settings)', { tag: ['@lite', '@customer'] }, async () => {
        await dbUtils.setOptionValue(dbData.dokan.optionName.general, { ...dbData.dokan.generalSettings, store_products_per_page: '1' });
    ...
    test('admin can enable address fields on registration (general settings)', { tag: ['@lite', '@guest'] }, async () => {
        await dbUtils.setOptionValue(dbData.dokan.optionName.general, { ...dbData.dokan.generalSettings, enabled_address_on_reg: 'on' });
        await guest.enableAddressFieldsOnRegistration('on');

        await dbUtils.setOptionValue(dbData.dokan.optionName.general, { ...dbData.dokan.generalSettings, enabled_address_on_reg: 'off' });
    ...
    test('admin can enable store terms and conditions on registration (general settings)', { tag: ['@pro', '@vendor'] }, async () => {
        await dbUtils.setOptionValue(dbData.dokan.optionName.general, { ...dbData.dokan.generalSettings, enable_tc_on_reg: 'on' });
        await vendor.enableStoreTermsAndConditionsOnRegistration('on');

        await dbUtils.setOptionValue(dbData.dokan.optionName.general, { ...dbData.dokan.generalSettings, enable_tc_on_reg: 'off' });
    ...
    test('admin can set show vendor info (general settings)', { tag: ['@lite', '@customer'] }, async () => {
        await dbUtils.setOptionValue(dbData.dokan.optionName.general, { ...dbData.dokan.generalSettings, show_vendor_info: 'on' });
        await customer.setShowVendorInfo(data.predefined.simpleProduct.product1.name, 'on');

        await dbUtils.setOptionValue(dbData.dokan.optionName.general, { ...dbData.dokan.generalSettings, show_vendor_info: 'off' });
    ...
    test('admin can enable more products tab (general settings)', { tag: ['@lite', '@customer'] }, async () => {
        await dbUtils.setOptionValue(dbData.dokan.optionName.general, { ...dbData.dokan.generalSettings, enabled_more_products_tab: 'on' });
        await customer.enableMoreProductsTab(data.predefined.simpleProduct.product1.name, 'on');

        await dbUtils.setOptionValue(dbData.dokan.optionName.general, { ...dbData.dokan.generalSettings, enabled_more_products_tab: 'off' });
    ...
    test('admin can enable vendor selling (selling settings)', { tag: ['@lite', '@guest'] }, async () => {
        await dbUtils.setOptionValue(dbData.dokan.optionName.selling, { ...dbData.dokan.sellingSettings, new_seller_enable_selling: 'on' });
        await guest.enableVendorSelling('on');

        await dbUtils.setOptionValue(dbData.dokan.optionName.selling, { ...dbData.dokan.sellingSettings, new_seller_enable_selling: 'off' });
    ...
    test('admin can set order status change capability (selling settings)', { tag: ['@lite', '@vendor'] }, async () => {
        ...
        await dbUtils.setOptionValue(dbData.dokan.optionName.selling, { ...dbData.dokan.sellingSettings, order_status_change: 'on' });
        await vendor.setOrderStatusChangeCapability(orderId, 'on');

        await dbUtils.setOptionValue(dbData.dokan.optionName.selling, { ...dbData.dokan.sellingSettings, order_status_change: 'off' });
