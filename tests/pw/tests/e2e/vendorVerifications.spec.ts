...
test.describe('Verifications test', () => {
    ...
    test('vendor verifications settings menu page is rendering properly', { tag: ['@pro', '@exp', '@v'] }, async () => {
        await vendor.vendorVerificationsSettingsRenderProperly();
    });

    test('vendor can send id verification request', { tag: ['@pro', '@v'] }, async () => {
        await vendor.sendIdVerificationRequest(data.vendor.verification);
    });

    test('vendor can send address verification request', { tag: ['@pro', '@v'] }, async () => {
        await vendor.sendAddressVerificationRequest(data.vendor.verification);
    });

    test('vendor can send company verification request', { tag: ['@pro', '@v'] }, async () => {
    ...
    test('admin verifications menu page is rendering properly', { tag: ['@pro', '@exp', '@a'] }, async () => {
        await admin.adminVerificationsRenderProperly();
    });

    // test('admin can approve ID verification request', { tag: ['@pro', '@a'] }, async ( ) => {
    // 	await admin.approveVerificationRequest(data.predefined.vendorInfo.username, 'id', 'approve');
    // });

    // test('admin can approve address verification request', { tag: ['@pro',  '@a'] }, async ( ) => {
    // 	await admin.approveVerificationRequest(data.predefined.vendorInfo.username, 'address', 'approve');
    // });

    // test('admin can approve company verification request', { tag: ['@pro',  '@a'] }, async ( ) => {
    // 	await admin.approveVerificationRequest(data.predefined.vendorInfo.username, 'company', 'approve');
    // });

    // todo: admin can reject requests

    // test('admin can disapprove approved ID verification request', { tag: ['@pro',  '@a'] }, async ( ) => {
    // 	await admin.disapproveVerificationRequest(data.predefined.vendorInfo.username, 'id');
    // });

    // test('admin can disapprove approved address verification request', { tag: ['@pro',  '@a'] }, async ( ) => {
    // 	await admin.disapproveVerificationRequest(data.predefined.vendorInfo.username, 'address');
    // });

    // test('admin can disapprove approved company verification request', { tag: ['@pro',  '@a'] }, async ( ) => {
