import { ANON, errorCode, siteUrl } from './paypalMarketplaceShared';
...
import { VENDOR_ID, PAYPAL_MERCHANTS, PAYPAL_VENDOR_LOGINS, hasCredentials, ensurePayPalConfigured, ensureClassicCheckoutPage, getPayPalStatus, getOrderMetaValue, seedPayPalConnectedVendor, probe, deleteOrder } from './helpers';
...
export const TRACKING = PayPalMarketplacePage.tracking;
 ...
 */
export const brief = (value: unknown, max = 300): string => String(JSON.stringify(value ?? null) ?? 'null').slice(0, max);
 ...
 */
export async function assertSessionIsAnonymous(label: string, headers: Headers = {}): Promise<void> {
...
export const GUEST_CHECKOUT_OPTION = 'woocommerce_enable_guest_checkout';

export const SIGNUP_AT_CHECKOUT_OPTION = 'woocommerce_enable_signup_and_login_from_checkout';
...
export const ORDER_FIELDS = 'id,parent_id,customer_id,status,total,order_key,payment_method,billing';
 ...
 */
export async function findOrderByBillingEmail(email: string): Promise<OrderRecord | null> {
 ...
 */
export async function latestOrderId(): Promise<number> {
...
export async function getOrder(orderId: string | number): Promise<OrderRecord | null> {
...
export interface GuestCheckoutOutcome {
 ...
 */
export async function placeGuestOrder(browser: Browser, productId: string, email: string, opts: { createAccount?: boolean } = {}): Promise<GuestCheckoutOutcome> {
 ...
 */
export async function ensureOrderTrackingPage(): Promise<void> {
 ...
 */
export async function readFlagOption(option: string): Promise<string | null> {
...
export const createdOrderIds: Array<string | number> = [];

export const createdUserIds: number[] = [];

export let previousGuestCheckout: string | null = null;

export let previousSignupAtCheckout: string | null = null;

export class PayPalMarketplaceGuestPage {
    ...
    async ppGst01({ browser }: { browser: Browser }): Promise<void> {
        test.skip(
            !hasCredentials,
    ...
    async ppGst02({ browser }: { browser: Browser }): Promise<void> {
        test.skip(
            !hasCredentials,
    ...
    async ppGst03({ browser }: { browser: Browser }): Promise<void> {
        test.skip(
            !hasCredentials,
    ...
    async ppGst04({ browser }: { browser: Browser }): Promise<void> {
        test.skip(
            !hasCredentials,
