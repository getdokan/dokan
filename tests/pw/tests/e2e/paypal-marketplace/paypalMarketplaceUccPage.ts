import { CHECKOUT_AJAX, PERSISTENT_CART_META, VENDOR1_EMAIL, VENDOR2_EMAIL } from './paypalMarketplaceShared';
...
import { customerAuth, VENDOR_ID, VENDOR2_ID, CUSTOMER_ID, PAYPAL_MERCHANTS, hasCredentials, ensurePayPalConfigured, getPayPalStatus, seedPayPalConnectedVendor, ensureCustomerAddress, ensureVendorStoreAddress, ensureClassicCheckoutPage, getBothMerchantConsents, isPayableMerchant, getUccState, setUccGate, setVendorUcc, adminContext, getWcOrder, getDokanOrderRows, near, describeProbe, cardFormMarkers } from './helpers';
...
export const HOSTED_FIELD = PayPalMarketplacePage.hostedField;
 ...
 */
export const UCC_VENDORS: string[] = [VENDOR_ID, VENDOR2_ID];
...
export const HAS_TEST_CARD = /^\d{12,19}$/.test(CARD.number);
...
export function eligibilitySkipReason(diagnostic: Record<string, unknown>): string {
    return (
...
export function environmentSkipReason(state: UccStateResolved): string {
    return (
...
export function metaOf(order: WcOrder, key: string): unknown {
...
export function metaString(order: WcOrder, key: string): string {
...
export function describeOrderMeta(order: WcOrder): string {
...
export function settled<T>(promise: Promise<T>): Promise<Settled<T>> {
    return promise.then(
 ...
 * unknown baseline.
 */
export let gateBaseline: { ucc_mode: string | null; button_type: string | null } | undefined;

/** The gate-5 meta as it was found, per vendor id. `''` means the row was absent. */
export let vendorUccBaseline: Record<string, string> | undefined;

/** Open all five local gates for the given sellers. Country and currency are asserted, never written. */
export async function openUccGates(vendorIds: string[]): Promise<void> {
 ...
 */
export async function restoreUccBaseline(): Promise<void> {
    if (gateBaseline) {
...
export function resolvedOrSkip(state: UccState): UccStateResolved {
...
export function countryCurrencyOpen(state: UccStateResolved): boolean {
 ...
 */
export async function openGatesAndRequireUcc(vendorIds: string[]): Promise<UccStateResolved> {
...
export let cachedConsents: Record<'vendor1' | 'vendor2', MerchantConsent> | null = null;

export async function gateOnPayableMerchants(): Promise<void> {
    if (!cachedConsents) {
 ...
 */
export async function stockCart(page: Page, lines: CartLine[]): Promise<void> {
 ...
 */
export async function withCustomerCheckout<T>(browser: Browser, lines: CartLine[], body: (page: Page) => Promise<T>): Promise<T> {
...
export async function readProbe(page: Page): Promise<CardProbe> {
...
export function assertServerRenderedCardForm(probe: CardProbe): void {
    expect(
...
export function assertCardSurfaceAbsent(probe: CardProbe, closedGate: string, consequence: string): void {
    expect(
        probe.gatewayOffered,
 ...
 */
export async function selectPayPalAndRevealCard(page: Page): Promise<void> {
    await expect(
 ...
 */
export async function mountHostedFields(page: Page): Promise<void> {
...
export function hostedFieldInput(page: Page, field: { container: string; input: string }) {
 ...
 */
export async function typeCard(page: Page, values: { number: string; expiry: string; cvv: string; holder: string }): Promise<void> {
 ...
 */
export async function payWithCard(page: Page): Promise<CardPaymentResult> {
...
export const allTouchedOrderIds = new Set<string>();

/* -------------------------------------------------------------- */
/* Shared captured-order fixture                                   */
/* -------------------------------------------------------------- */

export interface CapturedCardOrder extends CardPaymentResult {
    parent: WcOrder;
    subOrders: WcOrder[];
    dokanRows: DokanOrderRow[];
}

export let sharedCardOrder: CapturedCardOrder | null = null;
 ...
 */
export let sharedCardError: unknown = null;
 ...
 */
export async function getCapturedCardOrder(browser: Browser): Promise<CapturedCardOrder> {
    if (sharedCardOrder) {
...
export async function gateOnCardFixture(): Promise<void> {
    test.skip(!HAS_TEST_CARD, CARD_SKIP);
    await gateOnPayableMerchants();
}

export class PayPalMarketplaceUccPage {
    ...
    async ppUcc02({ browser }: { browser: Browser }): Promise<void> {
    ...
    async ppUcc03({ browser }: { browser: Browser }): Promise<void> {
        await openGatesAndRequireUcc(UCC_VENDORS);
    ...
    async ppUcc04({ browser }: { browser: Browser }): Promise<void> {
        await openGatesAndRequireUcc(UCC_VENDORS);
        await gateOnCardFixture();
    ...
    async ppUcc05({ browser }: { browser: Browser }): Promise<void> {
        await openGatesAndRequireUcc(UCC_VENDORS);
        await gateOnCardFixture();
    ...
    async ppUcc06({ browser }: { browser: Browser }): Promise<void> {
        await openGatesAndRequireUcc([VENDOR_ID]);
        test.skip(!HAS_TEST_CARD, CARD_SKIP);
    ...
    async ppUcc07({ browser }: { browser: Browser }): Promise<void> {
        await openGatesAndRequireUcc(UCC_VENDORS);
