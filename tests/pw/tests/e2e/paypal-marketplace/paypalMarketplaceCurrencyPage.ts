import { CURRENCY_OPTION, PERSISTENT_CART_META, TEST_NS } from './paypalMarketplaceShared';
...
export const INJECTED_CURRENCY_OPTION = 'dokan_e2e_paypal_extra_currency';
 ...
 */
export const SUPPORTED_ALT_CURRENCY = 'EUR';
 ...
 */
export const UNSUPPORTED_CURRENCY = 'INR';
 ...
 */
export const ZERO_DECIMAL_CURRENCY = 'JPY';
 ...
 */
export async function testRequest<T>(method: 'get' | 'post', route: string, data?: Record<string, unknown>): Promise<T> {
...
export const currencySupport = () => testRequest<CurrencySupport>('get', '/currency-support');

export const purchaseUnitsFor = (orderId: number) => testRequest<PurchaseUnitsResult>('post', '/purchase-units', { order_id: orderId });
 ...
 */
export async function readStoreCurrency(): Promise<string> {
...
export async function setStoreCurrency(code: string): Promise<void> {
...
export async function setInjectedCurrency(code: string | null): Promise<void> {
    if (code === null) {
...
export async function createOrder(productIds: string[]): Promise<number> {
...
export const money = (value: string | undefined): number => Number(value ?? '0');
 ...
 */
export function breakdownSum(breakdown: Record<string, Money>): number {
...
export function allMoneyStrings(unit: PurchaseUnit): string[] {
...
export function allCurrencyCodes(unit: PurchaseUnit): string[] {
 ...
 */
export async function openCustomerCheckout(browser: Browser, productId: string): Promise<{ ctx: BrowserContext; page: Page }> {
 ...
 */
export async function loadCheckout(page: Page): Promise<void> {
...
export const gatewayOffered = async (page: Page): Promise<boolean> => (await page.locator(CHECKOUT.radio).count()) > 0;

/** One-shot convenience for the cases that do not need to keep the session open. */
export async function paypalOfferedAtCheckout(browser: Browser, productId: string): Promise<boolean> {
...
export async function withAdminPage<T>(browser: Browser, body: (page: Page) => Promise<T>): Promise<T> {
...
export let originalCurrency = SUPPORTED_CURRENCY;

export let vendor1ProductId = '';

export let vendor2ProductId = '';

export let vendor3ProductId = '';

export let vendor3Id = '';

/** Why PP-CUR-07 could not run, when it could not — surfaced as the skip reason, never swallowed. */
export let vendor3Failure = '';

export const createdOrders: number[] = [];

export class PayPalMarketplaceCurrencyPage {
    ...
    async ppCur01({ browser }: { browser: Browser }): Promise<void> {
    ...
    async ppCur02({ browser }: { browser: Browser }): Promise<void> {
        test.skip(!hasCredentials, CREDENTIALS_SKIP);
        const status = await getPayPalStatus();
        test.skip(!status.module_active, MODULE_SKIP);
    ...
    async ppCur04({ browser }: { browser: Browser }): Promise<void> {
        const status = await getPayPalStatus();
        test.skip(!status.module_active, MODULE_SKIP);
    ...
    async ppCur08({ browser }: { browser: Browser }): Promise<void> {
        test.skip(!hasCredentials, CREDENTIALS_SKIP);
        const status = await getPayPalStatus();
        test.skip(!status.module_active, MODULE_SKIP);
    ...
    async ppCur09({ browser }: { browser: Browser }): Promise<void> {
        test.skip(!hasCredentials, CREDENTIALS_SKIP);
        const status = await getPayPalStatus();
        test.skip(!status.module_active, MODULE_SKIP);
