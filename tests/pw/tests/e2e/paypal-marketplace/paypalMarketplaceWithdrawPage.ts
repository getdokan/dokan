import { VENDOR1_EMAIL } from './paypalMarketplaceShared';
...
import { VENDOR_ID, PAYPAL_MERCHANTS, hasCredentials, ensurePayPalConfigured, getPayPalStatus, seedPayPalConnectedVendor, clearPayPalVendor } from './helpers';
...
export const WITHDRAW_OPTION = 'dokan_withdraw';
 ...
 */
export const STOCK_METHODS = ['paypal', 'bank', 'dokan_custom', 'skrill'] as const;
 ...
 */
export const BALANCE_TOPUP_PARTICULARS = 'PP-WDR balance top-up';

/** `wp_dokan_vendor_balance`, resolved the same way every other spec resolves a raw table name. */
export const VENDOR_BALANCE_TABLE = `${process.env.DB_PREFIX ?? 'wp'}_dokan_vendor_balance`;
 ...
 */
export async function readWithdrawOption(): Promise<OptionMap> {
 ...
 */
export async function enabledWithdrawMethods(): Promise<string[]> {
 ...
 */
export async function setEnabledWithdrawMethods(keys: string[]): Promise<void> {
    const option = await readWithdrawOption();
...
export interface StorePaymentView {
 ...
 */
export async function readStorePaymentView(vendorId: string): Promise<StorePaymentView> {
...
export interface WithdrawProbe {
 ...
 */
export async function probePayPalWithdrawRequest(): Promise<WithdrawProbe> {
    const api = new ApiUtils(await request.newContext());
    try {
...
export async function readVendorBalance(): Promise<number> {
    const api = new ApiUtils(await request.newContext());
    try {
        const [, body] = await api.get(endPoints.getBalanceDetails, { headers: payloads.vendorAuth });
 ...
 */
export function manualWithdrawUnsupportedSkip(probe: WithdrawProbe): string {
    return (
...
export let withdrawOptionBaseline: OptionMap = {};

export class PayPalMarketplaceWithdrawPage {
    ...
    async ppWdr02({ browser }: { browser: Browser }): Promise<void> {
        test.skip(!hasCredentials, CREDENTIALS_SKIP);
    ...
    async ppWdr03({ browser }: { browser: Browser }): Promise<void> {
        test.skip(!hasCredentials, CREDENTIALS_SKIP);
    ...
    async ppWdr04({ browser }: { browser: Browser }): Promise<void> {
        test.skip(!hasCredentials, CREDENTIALS_SKIP);
    ...
    async ppWdr05({ browser }: { browser: Browser }): Promise<void> {
        test.skip(!hasCredentials, CREDENTIALS_SKIP);
