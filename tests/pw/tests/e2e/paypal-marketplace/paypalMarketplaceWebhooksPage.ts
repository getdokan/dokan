...
import { CUSTOMER_ID, VENDOR_ID, VENDOR2_ID, PAYPAL_GATEWAY_ID, PAYPAL_EVENTS, PAYPAL_UNHANDLED_EVENTS, PAYPAL_MERCHANTS, PAYPAL_WEBHOOK_URL, MODULE_PRODUCT_SUBSCRIPTION, hasCredentials, ensurePayPalConfigured, getPayPalStatus, seedPayPalConnectedVendor, injectPayPalWebhook, postUnsignedWebhook, getOrderMetaValue, getOrderNotes, getOrderStatus, setOrderMeta, armInterceptor, disarmInterceptor, readCapturedVerifyRequest } from './helpers';
...
export const UNKNOWN_EVENT = 'DOKAN.E2E.NO-SUCH.EVENT';

/** An order id that cannot exist, for PP-WHK-22. */
export const NONEXISTENT_ORDER_ID = 999_999_991;
 ...
 */
export const HAS_PAYPAL_SUBSCRIPTION_FIXTURE = false;
 ...
 */
export function completedOrderResource(orderId: number, paypalOrderId: string, captureId: string): Record<string, unknown> {
    return {
        id: paypalOrderId,
        intent: 'CAPTURE',
        status: 'COMPLETED',
 ...
 */
export function refundResource(orderId: number | string, refundId: string): Record<string, unknown> {
    return {
        id: refundId,
        status: 'COMPLETED',
        custom_id: String(orderId),
...
export async function refundRowsForOrder(orderId: number | string): Promise<Array<{ id: number; status: number; refund_amount: string }>> {
...
export async function balanceRowsForOrder(orderId: number | string, trnType: string): Promise<Array<{ id: number; credit: string; debit: string }>> {
...
export async function rawOptionValue(optionName: string): Promise<string | null> {
...
export async function rawPostMetaValue(postId: number | string, metaKey: string): Promise<string | null> {
 ...
 * authenticated one.
 */
export async function postRawToWebhookEndpoint(body: string): Promise<{ status: number; text: string }> {
...
export class PayPalMarketplaceWebhooksPage {
    ...
    // PP-WHK-05 / 06 — one body, two events. Case id, event and the recorded gap arrive as
    // arguments because these cases are declared from a matrix rather than one at a time.
    async ppWhkUnhandledEvent(caseId: string, eventType: string, gap: string): Promise<void> {
    ...
    // PP-WHK-14 / 15 — the CANCELLED and EXPIRED aliases of one handler.
    async ppWhkSubscriptionCancelled(caseId: string, eventType: string): Promise<void> {
...
// The two case matrices. They live here rather than in the spec so the spec carries test
// declarations only; the spec loops over them to declare one test per row.
export const UNHANDLED_EVENT_CASES = [
        [
            'PP-WHK-05',
...
export const SUBSCRIPTION_CANCEL_CASES = [
