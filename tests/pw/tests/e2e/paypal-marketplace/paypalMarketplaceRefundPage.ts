import { HAS_APPROVAL_ACTOR, HAS_BUYER_LOGIN } from './paypalMarketplaceShared';
...
import { PayPalMarketplacePage, PAYPAL_IDS, withCustomer, withVendor, waitForCheckoutSettled, selectClassicPayPal as selectClassicPayPalShared, submitClassicCheckout, stockCartWithProof, createBlockPayment, driveHostedApproval as driveHostedApprovalShared } from './paypalMarketplacePage';
...
import { vendorAuth, VENDOR_ID, VENDOR2_ID, CUSTOMER_ID, PAYPAL_MERCHANTS, hasCredentials, HAS_REAL_MERCHANTS, ensurePayPalConfigured, getPayPalStatus, seedPayPalConnectedVendor, getBothMerchantConsents, isPayableMerchant, ensureCustomerAddress, ensureVendorStoreAddress, ensureClassicCheckoutPage, getOrderMetaValue, getOrderNotes, getOrderStatus, setOrderStatus, refundOrderViaDokan, readPayPalOrder } from './helpers';
 ...
 */
export const CAPTURE_PAYMENT_ROUTE = '/dokan/v1/paypal-marketplace/capture-payment';

/** WooCommerce's own Store API — the transport the checkout block places an order with. */
export const STORE_API_CART = '/wc/store/v1/cart';

export const STORE_API_CHECKOUT = '/wc/store/v1/checkout';
 ...
 */
export const RETURN_TO_SITE = /order-received/i;
 ...
 */
export const STORE_API_SHIPPING_REFUSAL = /no shipping options were found|invalid shipping option|shipping option is invalid/i;

/** `wp_dokan_refund.status` codes (`dokan-pro/includes/Refund/Refunds.php:11-13`). */
export const REFUND_STATUS = { pending: 0, completed: 1, cancelled: 2 } as const;
 ...
 */
export const DOK_018_SYMPTOM = /No PayPal capture id is found/i;

export const DOK_019_SYMPTOM = /DUPLICATE_INVOICE_ID/i;

/** Any PayPal-side refusal of a refund (`includes/Refund.php:154-161`). */
export const REFUND_API_ERROR = /API Refund Error/i;
 ...
 */
export const TOKEN_FAILURE = (status: number): string =>
...
export function money(value: PayPalMoney | undefined): number {
    return Number(value?.value ?? 0);
}

/** The purchase unit built from one (sub) order — `custom_id` is that order's id (`Order/OrderManager.php:219`). */
export function unitForOrder(paypalOrder: PayPalOrder, orderId: string): PayPalPurchaseUnit | null {
    return (paypalOrder.purchase_units ?? []).find(unit => String(unit.custom_id ?? '') === String(orderId)) ?? null;
}

export function capturesOf(unit: PayPalPurchaseUnit | null): PayPalCapture[] {
    return unit?.payments?.captures ?? [];
}

export function refundsOf(unit: PayPalPurchaseUnit | null): PayPalRefund[] {
...
export async function wcOrder(orderId: string | number): Promise<WcOrder> {
...
export function refundedTotal(order: WcOrder): number {
    return (order.refunds ?? []).reduce((sum, refund) => sum + Math.abs(Number(refund.total ?? 0)), 0);
}

export interface DokanRefundRow {
    id: number;
    order_id: number;
    seller_id: number;
...
export async function refundRows(orderId: string | number): Promise<DokanRefundRow[]> {
 ...
 */
export async function ledgerRows(vendorId: string | number): Promise<LedgerRow[]> {
    return (await dbUtils.dbQuery(
...
export async function reverseWithdrawalRows(orderId: string | number): Promise<Array<Record<string, unknown>>> {
    return (await dbUtils.dbQuery(
 ...
 */
export async function activeWithdrawStatuses(): Promise<string[]> {
 ...
 */
export async function ensureRefundApprovable(orderId: string): Promise<string> {
...
export async function waitForOrderMeta(orderId: string, key: string, timeoutMs = 90_000): Promise<string | undefined> {
 ...
 */
export async function blockApiPost(page: Page, route: string): Promise<CheckoutResult> {
...
export interface StoreApiCheckoutResult {
    order_id?: number | string;
    status?: string;
    payment_result?: {
 ...
 */
export async function storeApiPlaceOrder(page: Page, paypalOrderId: string): Promise<StoreApiCheckoutResult> {
 ...
 */
export async function driveHostedApprovalForRefund(page: Page, approvalUrl: string): Promise<void> {
 ...
 */
export async function approveOnPayPal(page: Page, approvalUrl: string): Promise<void> {
 ...
 */
export async function approveOnPayPalWithoutReturning(page: Page, approvalUrl: string, paypalOrderId: string): Promise<void> {
 ...
 */
export interface StoreApiPlacement {
...
export function merchantForVendor(vendorId: string): string {
 ...
 */
export async function placeThroughStoreApi(page: Page, paypal: PayPalMarketplacePage, ids: { parentOrderId: string; paypalOrderId: string }): Promise<StoreApiPlacement> {
 ...
 */
export async function captureOrder(browser: Browser, opts: { productIds: string[]; path?: 'classic' | 'block' | 'store' }): Promise<CapturedOrder> {
 ...
 */
export async function assertCaptureReconciles(captured: CapturedOrder): Promise<PayPalOrder> {
 ...
 */
export async function issueAutomaticRefund(orderId: string, amount?: number): Promise<IssuedRefund> {
 ...
 */
export function assertNoRefundFailureNote(notes: string[], orderId: string): void {
...
export async function readRefundFromPayPal(paypalOrderId: string, targetOrderId: string): Promise<{ order: PayPalOrder; unit: PayPalPurchaseUnit | null; refunds: PayPalRefund[] }> {
...
export interface VendorAjaxResult {
    status: number;
 ...
 */
export async function vendorRefundRequest(
    browser: Browser,
...
export async function adminRefundQueue(orderId: string, status?: 'pending' | 'completed' | 'cancelled'): Promise<AdminRefundItem[]> {
    const ctx = await request.newContext({ extraHTTPHeaders: payloads.adminAuth as Record<string, string> });
    try {
...
export async function adminRefundAction(refundId: number, action: 'approve' | 'cancel'): Promise<{ status: number; body: string }> {
    const ctx = await request.newContext({ extraHTTPHeaders: payloads.adminAuth as Record<string, string> });
    try {
...
export let gatePromise: Promise<string | null> | null = null;

export async function computeMoneyGate(): Promise<string | null> {
    if (!hasCredentials) {
...
export function moneyGate(): Promise<string | null> {
...
export let originalButtonType: string | null = null;

export let originalDisbursementMode: string | null = null;

export class PayPalMarketplaceRefundPage {
    ...
    async ppRef01({ browser }: { browser: Browser }): Promise<void> {
    ...
    async ppRef02({ browser }: { browser: Browser }): Promise<void> {
        const gate = await moneyGate();
        test.skip(gate !== null, gate ?? '');
    ...
    async ppRef03({ browser }: { browser: Browser }): Promise<void> {
        const gate = await moneyGate();
        test.skip(gate !== null, gate ?? '');
    ...
    async ppRef04({ browser }: { browser: Browser }): Promise<void> {
        const gate = await moneyGate();
        test.skip(gate !== null, gate ?? '');
    ...
    async ppRef05({ browser }: { browser: Browser }): Promise<void> {
        const gate = await moneyGate();
        test.skip(gate !== null, gate ?? '');
    ...
    async ppRef06({ browser }: { browser: Browser }): Promise<void> {
        const gate = await moneyGate();
        test.skip(gate !== null, gate ?? '');
    ...
    async ppRef07({ browser }: { browser: Browser }): Promise<void> {
        const gate = await moneyGate();
        test.skip(gate !== null, gate ?? '');
    ...
    async ppRef08({ browser }: { browser: Browser }): Promise<void> {
        const gate = await moneyGate();
        test.skip(gate !== null, gate ?? '');
    ...
    async ppRef09({ browser }: { browser: Browser }): Promise<void> {
        const gate = await moneyGate();
        test.skip(gate !== null, gate ?? '');
    ...
    async ppRef10({ browser }: { browser: Browser }): Promise<void> {
        const gate = await moneyGate();
        test.skip(gate !== null, gate ?? '');
    ...
    async ppRef11({ browser }: { browser: Browser }): Promise<void> {
        const gate = await moneyGate();
        test.skip(gate !== null, gate ?? '');
    ...
    async ppRef12({ browser }: { browser: Browser }): Promise<void> {
        const gate = await moneyGate();
        test.skip(gate !== null, gate ?? '');
    ...
    async ppRef13({ browser }: { browser: Browser }): Promise<void> {
        const gate = await moneyGate();
        test.skip(gate !== null, gate ?? '');
    ...
    async ppRef14({ browser }: { browser: Browser }): Promise<void> {
        const gate = await moneyGate();
        test.skip(gate !== null, gate ?? '');
