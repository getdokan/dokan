import { ADMIN_AJAX_URL, ANON, CUSTOMER, VENDOR2, errorCode, siteUrl } from './paypalMarketplaceShared';
 ...
 *
 *   template_redirect (source coordinates withheld)
 ...
 */
export const VENDOR2_PRODUCT_PRICE = 17;


export const ADMIN: Headers = payloads.adminAuth as Headers;


export const VENDOR: Headers = payloads.vendorAuth as Headers;
...
export const ajaxErrorType = (p: Probe): string => {
...
export async function readUserMetaRaw(userId: string | number, metaKey: string): Promise<string | null> {
...
export async function countRefundRows(orderId: string | number): Promise<number> {
...
export function purchaseUnitsOf(ppOrder: Record<string, unknown>): PayPalPurchaseUnit[] {
 ...
 */
export async function readPayPalOrderId(orderId: string): Promise<string> {
 ...
 */
export async function unpayableMerchantsReason(): Promise<string | null> {
...
export const CREATE_ORDER_KEYS_SKIP =
...
export async function createOrder(opts: { customerId: number; productId: string | string[]; status?: string }): Promise<{ id: string; total: number }> {
...
export async function getOrderTotal(orderId: string | number): Promise<number> {
...
export async function getSubOrders(parentId: string | number): Promise<Array<{ id: number; total: string; vendorId: string }>> {
 ...
 * `window.dokan_paypal.nonce` (the checkout bootstrap). It only exists when the gateway is
 * enabled AND the button type is `smart`, so callers must treat '' as "not available here".
 */
export async function readCheckoutNonce(page: Page): Promise<string> {
...
export async function collectRenderedText(page: Page, url: string): Promise<string> {
    const chunks: string[] = [];
...
export let customerOrder = { id: '', total: 0 };

export let victimOrder = { id: '', total: 0 };

export let vendor1MerchantId = '';

export let merchantIdKey = '';

export let marketplaceSettingsKey = '';

export let vendorShopUrl = '';

export const createdOrderIds: string[] = [];

export class PayPalMarketplaceSecurityPage {
    ...
    async ppSec02(): Promise<void> {
        const before = await getOrderStatus(victimOrder.id);

        // the logged-out AJAX capture action. Its ONLY guard
        ...
        log.warn(
            ...
            `the logged-out AJAX capture action is reachable by any visitor. Its only guard is its single validation helper: wp_verify_nonce against 'dokan_paypal_checkout_nonce' — a nonce minted for EVERY checkout visitor at the checkout bootstrap — plus a non-empty $_POST['order_id'] that is never checked for ownership. The REST twin of this operation gained check_order_permission() in 5.0.8; the AJAX twin did not. Filed in the private security tracker.`,
        );
    }

    async ppSec03({ browser }: { browser: Browser }): Promise<void> {
        const before = await getOrderStatus(victimOrder.id);
    ...
    async ppSec09({ browser }: { browser: Browser }): Promise<void> {
        ...
        // (source coordinates withheld), so admin-ajax answers its bare `0`.
        ...
        try {
            ...
            expect(forged.text, 'the refusal must come from the nonce check (source coordinates withheld), naming nonce verification').toMatch(/cheating|Nonce Verification Failed/i);
        } finally {
            await ctx.close();
        }
    ...
    async ppSec10({ browser }: { browser: Browser }): Promise<void> {
        /*
    ...
    async ppSec11({ browser }: { browser: Browser }): Promise<void> {
        test.skip(
            !hasCredentials,
    ...
    async ppSec13({ browser }: { browser: Browser }): Promise<void> {
        test.skip(
            !hasCredentials,
    ...
    async ppSec15({ browser }: { browser: Browser }): Promise<void> {
        const before = await readUserMetaRaw(VENDOR_ID, merchantIdKey);
        expect(
            before,
        ...
        try {
            ...
            expect(
                after,
                `a nonce-less GET overwrote vendor ${VENDOR_ID}'s PayPal merchant id from "${String(before)}" to "${String(after)}". CONFIRMED defect DOK-029 (CRITICAL), verified against dokan-pro 5.0.9 source: the three guards at the affected handler are written "<guard shape withheld>", the guard shape and its source coordinates are recorded in the private security tracker.`,
            ).toBe(before);
        } finally {
