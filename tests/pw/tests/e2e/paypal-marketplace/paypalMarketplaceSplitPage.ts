import { HAS_APPROVAL_ACTOR, PERSISTENT_CART_META } from './paypalMarketplaceShared';
...
import { PAYPAL_IDS, withCustomer, waitForCheckoutSettled, selectClassicPayPal as selectClassicPayPalOnCheckout, submitClassicCheckout, driveHostedApproval, paypalDiagnostics } from './paypalMarketplacePage';
import type { ClassicCheckoutResult } from './paypalMarketplacePage';
import { VENDOR_ID, VENDOR2_ID, CUSTOMER_ID, PAYPAL_MERCHANTS, hasCredentials, HAS_REAL_MERCHANTS, ensurePayPalConfigured, getPayPalStatus, seedPayPalConnectedVendor, getBothMerchantConsents, isPayableMerchant, ensureCustomerAddress, ensureVendorStoreAddress, ensureClassicCheckoutPage, getOrderMetaValue, readPayPalOrder, adminContext, getWcOrder, getDokanOrderRows, amountOf, near } from './helpers';
...
export const MERCHANT_BY_VENDOR: Record<string, string> = {
 ...
 */
export type Consents = Awaited<ReturnType<typeof getBothMerchantConsents>>;

export let cachedConsents: Consents | null = null;

export async function unpayableMerchantReasons(): Promise<string[]> {
    if (!cachedConsents) {
...
export function unitBreakdown(unit: PayPalPurchaseUnit): Record<string, string> {
...
export function describeUnit(unit: PayPalPurchaseUnit): Record<string, unknown> {
    return {
...
export function describeUnits(order: PayPalOrder): Array<Record<string, unknown>> {
...
export function vendorIdOf(order: WcOrder): string {
 ...
 */
export function expectedTaxTotal(order: WcOrder): number {
 ...
 */
export function expectedItemTotal(order: WcOrder): number {
...
export function adminCommissionOf(row: DokanOrderRow): number {
...
export type ProcessPaymentResult = ClassicCheckoutResult;
 ...
 */
export async function createPayPalOrderForExistingOrder(page: Page, orderId: string | number): Promise<ProcessPaymentResult> {
 ...
 */
export async function applyCouponAtCheckout(page: Page, code: string): Promise<string> {
 ...
 */
export async function approveAsBuyerOnPayPal(page: Page, approveUrl: string): Promise<void> {
...
export let vendorCouponId = '';

export let vendorCouponCode = '';

/** Admin-funded coupon for PP-SPL-05 — the only supported configuration that subsidises a vendor. */
export let adminCouponId = '';

export let adminCouponCode = '';
 ...
 */
export async function buildSplitOrder(browser: Browser, lines: CartLine[], opts: { couponCode?: string; cardCapture?: boolean } = {}): Promise<SplitOrder> {
 ...
 */
export let sharedSplit: SplitOrder | null = null;

export let sharedSplitError: Error | null = null;

export async function getSharedSplitOrder(browser: Browser): Promise<SplitOrder> {
    if (sharedSplit) {
        return sharedSplit;
    }
    if (sharedSplitError) {
 ...
 */
export function subOrderForUnit(unit: PayPalPurchaseUnit, split: SplitOrder): WcOrder | undefined {
 ...
 */
export async function readVendorBalance(auth: Record<string, string>): Promise<string | null> {
...
export class PayPalMarketplaceSplitPage {
    ...
    async ppSpl01({ browser }: { browser: Browser }): Promise<void> {
    ...
    async ppSpl02({ browser }: { browser: Browser }): Promise<void> {
        test.skip(!hasCredentials, CREDENTIALS_SKIP);
        test.skip(!HAS_REAL_MERCHANTS, MERCHANT_SKIP);
        await gateOnPayableMerchants();
    ...
    async ppSpl03({ browser }: { browser: Browser }): Promise<void> {
        test.skip(!hasCredentials, CREDENTIALS_SKIP);
        test.skip(!HAS_REAL_MERCHANTS, MERCHANT_SKIP);
        await gateOnPayableMerchants();
    ...
    async ppSpl04({ browser }: { browser: Browser }): Promise<void> {
        test.skip(!hasCredentials, CREDENTIALS_SKIP);
        test.skip(!HAS_REAL_MERCHANTS, MERCHANT_SKIP);
        await gateOnPayableMerchants();
    ...
    async ppSpl05({ browser }: { browser: Browser }): Promise<void> {
        test.skip(!hasCredentials, CREDENTIALS_SKIP);
        test.skip(!HAS_REAL_MERCHANTS, MERCHANT_SKIP);
        await gateOnPayableMerchants();
        test.skip(
    ...
    async ppSpl06({ browser }: { browser: Browser }): Promise<void> {
        test.skip(!hasCredentials, CREDENTIALS_SKIP);
        test.skip(!HAS_REAL_MERCHANTS, MERCHANT_SKIP);
        await gateOnPayableMerchants();
    ...
    async ppSpl07({ browser }: { browser: Browser }): Promise<void> {
        test.skip(!hasCredentials, CREDENTIALS_SKIP);
        test.skip(!HAS_REAL_MERCHANTS, MERCHANT_SKIP);
        await gateOnPayableMerchants();
    ...
    async ppSpl08({ browser }: { browser: Browser }): Promise<void> {
        test.skip(!hasCredentials, CREDENTIALS_SKIP);
        test.skip(!HAS_REAL_MERCHANTS, MERCHANT_SKIP);
        await gateOnPayableMerchants();
    ...
    async ppSpl09({ browser }: { browser: Browser }): Promise<void> {
        test.skip(!hasCredentials, CREDENTIALS_SKIP);
        test.skip(!HAS_REAL_MERCHANTS, MERCHANT_SKIP);
        await gateOnPayableMerchants();
    ...
    async ppSpl10({ browser }: { browser: Browser }): Promise<void> {
        test.skip(!hasCredentials, CREDENTIALS_SKIP);
        test.skip(!HAS_REAL_MERCHANTS, MERCHANT_SKIP);
        await gateOnPayableMerchants();
    ...
    async ppSpl11({ browser }: { browser: Browser }): Promise<void> {
        test.skip(!hasCredentials, CREDENTIALS_SKIP);
        test.skip(!HAS_REAL_MERCHANTS, MERCHANT_SKIP);
        await gateOnPayableMerchants();
    ...
    async ppSpl12({ browser }: { browser: Browser }): Promise<void> {
        test.skip(!hasCredentials, CREDENTIALS_SKIP);
        test.skip(!HAS_REAL_MERCHANTS, MERCHANT_SKIP);
