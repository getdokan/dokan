...
## Layout note (2026-08-14) — read this before §1

This document was authored on the wave branches, where every converted feature
lived in its own `tests/e2e/new-<feature>/` folder. **On `develop` the merge
co-located those files with the legacy spec instead**, so the landed convention
is:

    tests/e2e/<feature>/new<Feature>.spec.ts
    tests/e2e/<feature>/new<Feature>Page.ts

i.e. `new-withdraw/newWithdrawPage.ts` is now `withdraws/newWithdrawPage.ts`,
`new-orders/` is now `orders/newOrders*.ts`, `new-shipping/` is now
`vendor-shipping/newShipping*.ts`. Read every `new-<feature>/` path below as its
co-located equivalent. **Everything else in this document still holds unchanged**
— above all §1's rule that a feature's spec files never import from another
feature's folder, only from `@utils/*`. Section numbers are unchanged because
page objects cite them.

Migration status and the per-case backlog live in
`tests/pw/NEW-UI-MIGRATION-CHECKLIST.md`.
