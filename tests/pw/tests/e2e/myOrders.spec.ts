...
test.describe('My orders functionality test', () => {
    ...
    test('customer can view order note', { tag: ['@lite', '@customer'] }, async () => {
        ...
        const [, orderId] = await apiUtils.createOrderNote(PRODUCT_ID, { ...payloads.createOrder, customer_id: CUSTOMER2_ID }, { ...payloads.createOrderNoteForCustomer, note: orderNote }, payloads.vendorAuth);
