...
test.describe('Dokan pro feature promo test', () => {
    ...
    test('dokan notice is rendering properly', { tag: ['@lite', '@exp', '@a'] }, async () => {
        await admin.dokanNoticeRenderProperly();
    });

    test('dokan promotion is rendering properly', { tag: ['@lite', '@exp', '@a'] }, async () => {
        await admin.dokanPromotionRenderProperly();
    });

    test('dokan pro features promotions are rendering properly', { tag: ['@liteOnly', '@exp', '@a'] }, async () => {
