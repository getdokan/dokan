...
test.describe('Vendor settings test', () => {
    ...
    test('vendor can set catalog settings', { tag: ['@lite', '@vendor'] }, async () => {
        ...
        // disable catalog
        await dbUtils.setOptionValue(dbData.dokan.optionName.selling, { ...dbData.dokan.sellingSettings, catalog_mode_hide_add_to_cart_button: 'off', catalog_mode_hide_product_price: 'off' });
    });
    ...
    test('vendor can set min-max settings', { tag: ['@pro', '@vendor'] }, async () => {
        ...
        // disable min-max
        await dbUtils.setOptionValue(dbData.dokan.optionName.selling, { ...dbData.dokan.sellingSettings, enable_min_max_quantity: 'off', enable_min_max_amount: 'off' });
    });
