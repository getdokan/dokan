...
test.describe('Request for quotation test admin', () => {
    ...
    test('admin can add quote @pro', async () => {
        await admin.addQuote({
            ...data.requestForQuotation.quote,
            title: data.requestForQuotation.quote.title(),
        });
    });

    test('admin can edit quote @pro', async () => {
        await admin.editQuote({
            ...data.requestForQuotation.quote,
            title: quoteTitle,
        });
    });
    ...
    test('admin can restore quote @pro', async () => {
        const [, , quoteTitle] = await apiUtils.createQuoteRequest(
            {
                ...payloads.createQuoteRequest(),
                product_ids: productId,
                status: 'trash',
...
test.describe('Request for quotation test vendor', () => {
    ...
    test('vendor can update quote request @pro', async () => {
        await vendor.vendorUpdateQuoteRequest(quoteId, {
            ...data.requestForQuotation.vendorUpdateQuote,
...
test.describe('Request for quotation test customer', () => {
    ...
    test.beforeAll(async ({ browser, request }) => {
        ...
        [, quoteId] = await apiUtils.createQuoteRequest(
            {
                ...payloads.createQuoteRequest(),
                product_ids: productId,
                user_id: CUSTOMER_ID,
    ...
    test('customer can update quote request @pro', async () => {
        await customer.customerUpdateRequestedQuote(quoteId, {
            ...data.requestForQuotation.customerQuoteProduct,
            productName: productName,
        });
    });
    ...
    test.skip('customer can quote product @pro @explo', async () => {
        await customer.customerQuoteProduct({
            ...data.requestForQuotation.customerQuoteProduct,
            productName: productName,
        });
    });
    ...
    test.skip('guest customer can quote product @pro @explo', async () => {
        await guest.customerQuoteProduct(
            {
                ...data.requestForQuotation.customerQuoteProduct,
                productName: productName,
            },
            data.requestForQuotation.guest(),
        );
    });
});
