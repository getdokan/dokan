...
test.describe('Store categories test', () => {
    ...
    test('admin store category page is rendering properly', { tag: ['@pro', '@exp', '@a'] }, async () => {
        await admin.adminStoreCategoryRenderProperly();
    });

    test('admin can add store category', { tag: ['@pro', '@a'] }, async () => {
        await admin.addStoreCategory(data.storeCategory());
    });

    test('admin can search store category', { tag: ['@pro', '@a'] }, async () => {
        await admin.searchStoreCategory(categoryName);
    });

    test('admin can edit store category', { tag: ['@pro', '@a'] }, async () => {
        await admin.editStoreCategory({ ...data.storeCategory(), name: categoryName });
    });

    test('admin can set default store category', { tag: ['@pro', '@a'] }, async () => {
    ...
    test('admin can delete store category', { tag: ['@pro', '@a'] }, async () => {
    ...
    test('vendor can update own store category', { tag: ['@pro', '@v'] }, async () => {
