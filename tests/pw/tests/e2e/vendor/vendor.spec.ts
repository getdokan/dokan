...
import { LoginPage, VendorPage, dbData, data } from './vendorPage';
...
test.describe('Vendor functionality test', () => {
    ...
    test('vendor can setup setup-wizard', { tag: ['@lite', '@vendor'] }, async () => { await vendor.vendorSetupWizard(data.vendorSetupWizard); });
    ...
    test('vendor can visit own Store', { tag: ['@lite', '@vendor'] }, async () => { await vendor.visitStore(data.predefined.vendorStores.vendor1); });
});
