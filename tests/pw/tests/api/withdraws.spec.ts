...
test.beforeAll(async ({ request }) => {
    ...
    const [responseBody, id] = await apiUtils.createWithdraw({
        ...payloads.createWithdraw,
        amount: minimumWithdrawLimit,
    });
    withdrawId =
        responseBody.message === 'You already have a pending withdraw request'
            ? await apiUtils.getWithdrawId()
            : id;
});
...
test.describe('withdraw api test', () => {
    ...
    test('get single withdraw @lite', async () => {
        const [response, responseBody] = await apiUtils.get(
            endPoints.getSingleWithdraw(withdrawId),
        );
        expect(response.ok()).toBeTruthy();
        expect(responseBody).toBeTruthy();
    });
    ...
    test('update batch withdraws @lite', async () => {
        const allWithdrawIds = (await apiUtils.getAllWithdraws()).map(
            (a: { id: unknown }) => a.id,
        );
        const [response, responseBody] = await apiUtils.put(
            endPoints.updateBatchWithdraws,
            { data: { approved: allWithdrawIds } },
        );
        expect(response.ok()).toBeTruthy();
        expect(responseBody).toBeTruthy();
    });
    ...
    test('create a withdraw @lite', async () => {
        ...
        const pendingRequest =
            await apiUtils.getAllWithdrawsByStatus('pending');
