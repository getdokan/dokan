...
test.beforeAll(async ({ request }) => {
    ...
    [productId, variationId] =
        await apiUtils.createVariableProductWithVariation(
            payloads.createAttribute(),
            payloads.createAttributeTerm(),
            payloads.createVariableProduct(),
        );
});
...
test.describe('product variation api test', () => {
    ...
    test('get single product variation @pro', async () => {
        const [response, responseBody] = await apiUtils.get(
            endPoints.getSingleProductVariation(productId, variationId),
        );
        expect(response.ok()).toBeTruthy();
        expect(responseBody).toBeTruthy();
    });
    ...
    test('update a product variation @pro', async () => {
        const [response, responseBody] = await apiUtils.put(
            endPoints.updateProductVariation(productId, variationId),
    ...
    test('delete a product variation @pro', async () => {
        const [response, responseBody] = await apiUtils.delete(
            endPoints.deleteProductVariation(productId, variationId),
        );
        expect(response.ok()).toBeTruthy();
        expect(responseBody).toBeTruthy();
    });
