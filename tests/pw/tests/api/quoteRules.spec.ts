...
test.beforeAll(async ({ request }) => {
    ...
    [, quoteRuleId] = await apiUtils.createQuoteRule(
...
test.describe('quote rules api test', () => {
    ...
    test('get single quote rule @pro', async () => {
        const [response, responseBody] = await apiUtils.get(
            endPoints.getSingleQuoteRule(quoteRuleId),
        );
        expect(response.ok()).toBeTruthy();
        expect(responseBody).toBeTruthy();
    });
    ...
    test('update a quote rule @pro', async () => {
        const [response, responseBody] = await apiUtils.put(
            endPoints.updateQuoteRule(quoteRuleId),
    ...
    test('restore a deleted quote rule @pro', async () => {
        const [response, responseBody] = await apiUtils.put(
            endPoints.restoreQuoteRule(quoteRuleId),
        );
        expect(response.ok()).toBeTruthy();
        expect(responseBody).toBeTruthy();
    });
    ...
    test('update batch quote rules @pro', async () => {
        const allQuoteRuleIds = (await apiUtils.getAllQuoteRules()).map(
            (a: { id: unknown }) => a.id,
        );
        const [response, responseBody] = await apiUtils.put(
