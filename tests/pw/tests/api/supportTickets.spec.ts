...
test.describe('support ticket api test', () => {
    test('get all support ticket customers @pro', async () => {
        const [response, responseBody] = await apiUtils.get(
            endPoints.getAllSupportTicketCustomers,
        );
    ...
    test('get single support ticket @pro', async () => {
        const [response, responseBody] = await apiUtils.get(
            endPoints.getSingleSupportTicket(supportTicketId),
    ...
    test('create a support ticket comment @pro', async () => {
        const [response, responseBody] = await apiUtils.post(
            ...
            { data: payloads.createSupportTicketComment },
        );
        expect(response.ok()).toBeTruthy();
        expect(responseBody).toBeTruthy();
    });
    ...
    test('update a support ticket status @pro', async () => {
        const [response, responseBody] = await apiUtils.post(
            ...
            { data: payloads.updateSupportTicketStatus },
        );
        expect(response.ok()).toBeTruthy();
        expect(responseBody).toBeTruthy();
    ...
    test('update a support ticket email notification @pro', async () => {
        const [response, responseBody] = await apiUtils.post(
            ...
            { data: payloads.updateSupportTicketEmailNotification },
        );
        expect(response.ok()).toBeTruthy();
        expect(responseBody).toBeTruthy();
    });
    ...
    test('delete a support ticket comment @pro', async () => {
        const supportTicketCommentId =
            await apiUtils.createSupportTicketComment(
                supportTicketId,
                payloads.createSupportTicketComment,
            );
        const [response, responseBody] = await apiUtils.delete(
            endPoints.deleteSupportTicketComment(supportTicketCommentId),
        );
        expect(response.ok()).toBeTruthy();
        expect(responseBody).toBeTruthy();
    });
    ...
    test('update batch support tickets @pro', async () => {
        const allSupportTicketIds = (await apiUtils.getAllSupportTickets()).map(
            (a: { ID: unknown }) => a.ID,
        );
        const [response, responseBody] = await apiUtils.put(
            endPoints.updateBatchSupportTickets,
            { data: { close: allSupportTicketIds } },
        );
        expect(response.ok()).toBeTruthy();
        expect(responseBody).toBeTruthy();
