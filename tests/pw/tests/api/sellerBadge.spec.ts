...
test.beforeAll(async ({ request }) => {
    ...
    [, badgeId] = await apiUtils.createSellerBadge(
...
test.describe('seller badge api test', () => {
    test('get verified-seller verification types @pro', async () => {
        const [response, responseBody] = await apiUtils.get(
            endPoints.getVerifiedSellerVerificationTypes,
        );
    ...
    test('get all seller badge events @pro', async () => {
        const [response, responseBody] = await apiUtils.get(
            endPoints.getAllSellerBadgeEvents,
        );
        expect(response.ok()).toBeTruthy();
        expect(responseBody).toBeTruthy();
    });
    ...
    test('get single seller badge @pro', async () => {
        const [response, responseBody] = await apiUtils.get(
            endPoints.getSingleSellerBadge(badgeId),
        );
        expect(response.ok()).toBeTruthy();
        expect(responseBody).toBeTruthy();
    });
    ...
    test('get vendor unseen seller badges @pro', async () => {
        const [response, responseBody] = await apiUtils.get(
            endPoints.getVendorUnseenSellerBadges,
    ...
    test('set vendor seller badges as seen @pro', async () => {
        const [response, responseBody] = await apiUtils.put(
            endPoints.setSellerBadgeAsSeen,
    ...
    test('update a seller badge @pro', async () => {
        const [response, responseBody] = await apiUtils.put(
            endPoints.updateSellerBadge(badgeId),
    ...
    test('update row actions @pro', async () => {
        const [response, responseBody] = await apiUtils.put(
            endPoints.setSellerBadgeRowActions,
    ...
    test('update batch seller badges @pro', async () => {
        const allBadgeIds = (await apiUtils.getAllSellerBadges()).map(
            (a: { id: unknown }) => a.id,
        );
        const [response, responseBody] = await apiUtils.put(
