...
test.beforeAll(async ({ request }) => {
    ...
    [, orderId, orderNoteId] = await apiUtils.createOrderNote(
        payloads.createProduct(),
        payloads.createOrder,
        payloads.createOrderNote,
    );
});
...
test.describe('order note api test', () => {
    test('get all order notes @lite', async () => {
        const [response, responseBody] = await apiUtils.get(
            endPoints.getAllOrderNotes(orderId),
        );
    ...
    test('get single order note @lite', async () => {
        const [response, responseBody] = await apiUtils.get(
            endPoints.getSingleOrderNote(orderId, orderNoteId),
        );
        expect(response.ok()).toBeTruthy();
        expect(responseBody).toBeTruthy();
    });
    ...
    test('delete an order note @lite', async () => {
        const [response, responseBody] = await apiUtils.delete(
            endPoints.deleteOrderNote(orderId, orderNoteId),
        );
        expect(response.ok()).toBeTruthy();
        expect(responseBody).toBeTruthy();
    });
