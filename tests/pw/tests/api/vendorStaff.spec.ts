...
test.beforeAll(async ({ request }) => {
    ...
    [, staffId] = await apiUtils.createVendorStaff(
        payloads.createStaff(),
        payloads.vendorAuth,
    );
...
test.describe('vendor staff api test', () => {
    ...
    test('get single vendor staff @pro', async () => {
        const [response, responseBody] = await apiUtils.get(
            endPoints.getSingleVendorStaff(staffId),
        );
        expect(response.ok()).toBeTruthy();
        expect(responseBody).toBeTruthy();
    });
    ...
    test('update a vendor staff @pro', async () => {
        ...
        const [response, responseBody] = await apiUtils.put(
            endPoints.updateVendorStaff('100'),
    ...
    test('get vendor staff capabilities @pro', async () => {
        const [response, responseBody] = await apiUtils.get(
            endPoints.getVendorStaffCapabilities(staffId),
        );
        expect(response.ok()).toBeTruthy();
        expect(responseBody).toBeTruthy();
    });
    ...
    test('update vendor staff capabilities @pro', async () => {
        ...
        const [response, responseBody] = await apiUtils.put(
            endPoints.updateVendorStaffCapabilities(staffId),
    ...
    test.skip('delete a vendor staff @pro', async () => {
        const [response, responseBody] = await apiUtils.delete(
            endPoints.deleteVendorStaff(staffId),
        );
