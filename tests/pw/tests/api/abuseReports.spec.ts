...
test.describe('abuse report api test', () => {
    test('get all abuse report reasons @pro', async () => {
        const [response, responseBody] = await apiUtils.get(
            endPoints.getAllAbuseReportReasons,
        );
    ...
    test('delete a abuse report @pro', async () => {
        ...
        const [response, responseBody] = await apiUtils.delete(
            endPoints.deleteAbuseReport(abuseReportId),
        );
        expect(response.ok()).toBeTruthy();
        expect(responseBody).toBeTruthy();
    });
    ...
    test('delete batch abuse reports @pro', async () => {
        const allAbuseReportIds = (await apiUtils.getAllAbuseReports()).map(
            (a: { id: unknown }) => a.id,
        );
        const [response, responseBody] = await apiUtils.delete(
            endPoints.deleteBatchAbuseReports,
            { data: { items: allAbuseReportIds } },
        );
        expect(response.ok()).toBeTruthy();
        expect(responseBody).toBeTruthy();
    });
