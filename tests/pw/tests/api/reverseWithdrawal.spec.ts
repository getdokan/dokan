...
test.describe('reverse withdrawal api test', () => {
    test('get reverse withdrawal transaction types @lite', async () => {
        const [response, responseBody] = await apiUtils.get(
            endPoints.getReverseWithdrawalTransactionTypes,
        );
    ...
    test('get all reverse withdrawal store balance @lite', async () => {
        const [response, responseBody] = await apiUtils.get(
            endPoints.getAllReverseWithdrawalStoreBalance,
        );
        expect(response.ok()).toBeTruthy();
        expect(responseBody).toBeTruthy();
    });
    ...
    test('get all reverse withdrawal transactions @lite', async () => {
        ...
        const [response, responseBody] = await apiUtils.get(
            endPoints.getAllReverseWithdrawalTransactions,
            {
                params: {
    ...
    test('get reverse withdrawal vendor due status @lite', async () => {
        const [response, responseBody] = await apiUtils.get(
            endPoints.getReverseWithdrawalVendorDueStatus,
        );
        expect(response.ok()).toBeTruthy();
        expect(responseBody).toBeTruthy();
    });
    ...
    test('add reverse withdrawal payment product to cart @lite', async () => {
        const [response, responseBody] = await apiUtils.post(
            endPoints.getReverseWithdrawalAddProductToCart,
            { data: payloads.amountToPay, headers: payloads.vendorAuth },
        );
        expect(response.ok()).toBeTruthy();
        expect(responseBody).toBeTruthy();
    });
