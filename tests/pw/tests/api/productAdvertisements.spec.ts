...
test.beforeAll(async ({ request }) => {
    ...
    [, productAdvertisementId] = await apiUtils.createProductAdvertisement(
...
test.describe('product advertisement api test', () => {
    test('get all advertised product stores @pro', async () => {
        const [response, responseBody] = await apiUtils.get(
            endPoints.getAllProductAdvertisementStores,
        );
    ...
    test('create a product advertisement @pro', async () => {
        const [body, productId] = await apiUtils.createProduct(
            payloads.createProduct(),
        );
    ...
    test('expire a product advertisement @pro', async () => {
        const [response, responseBody] = await apiUtils.put(
            endPoints.expireProductAdvertisement(productAdvertisementId),
        );
        expect(response.ok()).toBeTruthy();
        expect(responseBody).toBeTruthy();
    });
    ...
    test('delete a product advertisement @pro', async () => {
        const [response, responseBody] = await apiUtils.delete(
            endPoints.deleteProductAdvertisement(productAdvertisementId),
        );
        expect(response.ok()).toBeTruthy();
        expect(responseBody).toBeTruthy();
    });
    ...
    test('update batch product advertisements @pro', async () => {
        const allProductAdvertisementIds = (
            await apiUtils.getAllProductAdvertisements()
        ).map((a: { id: unknown }) => a.id);
        const [response, responseBody] = await apiUtils.put(
            endPoints.updateBatchProductAdvertisements,
