...
test.describe('stores api test', () => {
    ...
    test('update a store @lite', async () => {
        const [response, responseBody] = await apiUtils.put(
            endPoints.updateStore(sellerId),
    ...
    test('get store current visitor @pro', async () => {
        const [response, responseBody] = await apiUtils.get(
            endPoints.getStoreCurrentVisitor,
        );
        expect(response.ok()).toBeTruthy();
        expect(responseBody).toBeTruthy();
    });
    ...
    test('get store stats @pro', async () => {
        const [response, responseBody] = await apiUtils.get(
            endPoints.getStoreStats(sellerId),
        );
        expect(response.ok()).toBeTruthy();
        expect(responseBody).toBeTruthy();
    });
    ...
    test('get store slug availability @lite', async () => {
        const [response, responseBody] = await apiUtils.get(
            endPoints.getStoresSlugAvaility,
        );
        expect(response.ok()).toBeTruthy();
        expect(responseBody).toBeTruthy();
    });
    ...
    test('get store categories @lite', async () => {
        const [response, responseBody] = await apiUtils.get(
            endPoints.getStoreCategories(sellerId),
        );
        expect(response.ok()).toBeTruthy();
        expect(responseBody).toBeTruthy();
    });
    ...
    test('get store products @lite', async () => {
        ...
        const [response, responseBody] = await apiUtils.get(
            endPoints.getStoreProducts(sId),
        );
        expect(response.ok()).toBeTruthy();
        expect(responseBody).toBeTruthy();
    });
    ...
    test('update a store status @lite', async () => {
        const [response, responseBody] = await apiUtils.put(
            endPoints.updateStoreStatus(sellerId),
    ...
    test('client contact store @lite', async () => {
        const [response, responseBody] = await apiUtils.post(
            endPoints.clientContactStore(sellerId),
    ...
    test('admin email store @pro', async () => {
        const [response, responseBody] = await apiUtils.post(
            endPoints.adminEmailStore(sellerId),
    ...
    test('update batch stores @lite', async () => {
        const allStoreIds = (await apiUtils.getAllStores()).map(
            (a: { id: unknown }) => a.id,
        );
        const [response, responseBody] = await apiUtils.put(
            endPoints.updateBatchStores,
