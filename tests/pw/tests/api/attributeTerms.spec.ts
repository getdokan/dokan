...
test.beforeAll(async ({ request }) => {
    ...
    [, attributeId, attributeTermId] = await apiUtils.createAttributeTerm(
        payloads.createAttribute(),
        payloads.createAttributeTerm(),
    );
});
...
test.describe('attribute term api test', () => {
    ...
    test('update an attribute term  @lite', async () => {
        const [response, responseBody] = await apiUtils.put(
            endPoints.updateAttributeTerm(attributeId, attributeTermId),
    ...
    test('delete an attribute term @lite', async () => {
        const [response, responseBody] = await apiUtils.delete(
            endPoints.deleteAttributeTerm(attributeId, attributeTermId),
        );
        expect(response.ok()).toBeTruthy();
        expect(responseBody).toBeTruthy();
    });
    ...
    test('update batch attribute terms @lite', async () => {
        const allAttributeTermIds = (
            await apiUtils.getAllAttributeTerms(attributeId)
        ).map((a: { id: unknown }) => a.id);
        ...
        const [response, responseBody] = await apiUtils.put(
            endPoints.updateBatchAttributeTerms(attributeId),
