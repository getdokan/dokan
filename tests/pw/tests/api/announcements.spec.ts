...
test.beforeAll(async ({ request }) => {
    ...
    [, announcementId] = await apiUtils.createAnnouncement(
        payloads.createAnnouncement(),
    );
});
...
test.describe('announcements api test', () => {
    ...
    test('get single announcement @pro', async () => {
        const [response, responseBody] = await apiUtils.get(
            endPoints.getSingleAnnouncement(announcementId),
        );
        expect(response.ok()).toBeTruthy();
        expect(responseBody).toBeTruthy();
    });
    ...
    test('restore a deleted announcement @pro', async () => {
        const [response, responseBody] = await apiUtils.put(
            endPoints.restoreDeletedAnnouncement(announcementId),
        );
        expect(response.ok()).toBeTruthy();
        expect(responseBody).toBeTruthy();
    });
    ...
    test('update batch announcements @pro', async () => {
        const allAnnouncementIds = (await apiUtils.getAllAnnouncements()).map(
            (a: { id: unknown }) => a.id,
        );
        const [response, responseBody] = await apiUtils.put(
