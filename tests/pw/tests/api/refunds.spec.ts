...
test.beforeAll(async ({ request }) => {
    ...
    [, orderResponseBody] = await apiUtils.createOrderWithStatus(
...
test.describe('refunds api test', () => {
    ...
    test('get all refunds by status @pro', async () => {
        const [response, responseBody] = await apiUtils.get(
            endPoints.getAllRefunds,
            { params: { status: 'pending' }, headers: payloads.vendorAuth },
    ...
    test('cancel a refund @pro', async () => {
        const [response, responseBody] = await apiUtils.put(
            endPoints.cancelRefund(refundId),
        );
        expect(response.ok()).toBeTruthy();
        expect(responseBody).toBeTruthy();
    });
    ...
    test('delete a refund @pro', async () => {
        const [response, responseBody] = await apiUtils.delete(
            endPoints.deleteRefund(refundId),
        );
        expect(response.ok()).toBeTruthy();
        expect(responseBody).toBeTruthy();
    });
    ...
    test('approve a refund @pro', async () => {
        ...
        const [response, responseBody] = await apiUtils.put(
            endPoints.approveRefund(refundId),
        );
        expect(response.ok()).toBeTruthy();
        expect(responseBody).toBeTruthy();
    });
    ...
    test('update batch refunds @pro', async () => {
        ...
        const allPendingRefundsIds = (
            await apiUtils.getAllRefunds('pending', payloads.vendorAuth)
        ).map((a: { id: unknown }) => a.id);
        const [response, responseBody] = await apiUtils.put(
            endPoints.updateBatchRefunds,
