...
test.describe('customers api test', () => {
    ...
    test('get single customer @pro', async () => {
        const [response, responseBody] = await apiUtils.get(
            endPoints.getSingleCustomer(customerId),
        );
        expect(response.ok()).toBeTruthy();
        expect(responseBody).toBeTruthy();
    });
    ...
    test('update a customer @pro', async () => {
        const [response, responseBody] = await apiUtils.put(
            endPoints.updateCustomer(customerId),
    ...
    test('update batch customers @pro', async () => {
        const allCustomerIds = (await apiUtils.getAllCustomers()).map(
            (a: { id: unknown }) => a.id,
        );
        ...
        const [response, responseBody] = await apiUtils.put(
            endPoints.updateBatchCustomers,
