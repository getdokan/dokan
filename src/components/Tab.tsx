...
import { TabPanel, IconType, Slot } from '@wordpress/components';
...
const applyFiltersToTabElements = (namespace: string, elementName: string, element: any, props: Partial<DokanTabProps>) => {
    const snakeCaseNamespace = snakeCase(namespace);
    return window.wp.hooks.applyFilters(`dokan_${snakeCaseNamespace}_tab_${elementName}`, element, { ...props });
};
...
const DokanTab = ({
    ...
    const filteredProps = {
        ...props,
        tabs: applyFiltersToTabElements(namespace, 'tabs', tabsWithDefaultStyle, { ...props, tabs }),
        activeClass: applyFiltersToTabElements(namespace, 'active_class', activeClass, { ...props, tabs }),
        className: applyFiltersToTabElements(namespace, 'panel_class', twMerge(config.className, className), { ...props, tabs }),
    };
    ...
    return (
        <div id={`dokan-tab-${tabNameSpace}`} className="dokan-tab-wrapper">
            ...
            <Slot name={`dokan-before-tab-${tabNameSpace}`} fillProps={{ ...filteredProps }} />
            ...
            <Slot name={`dokan-after-tab-${tabNameSpace}`} fillProps={{ ...filteredProps }} />
        </div>
    );
};
