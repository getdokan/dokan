...
const DokanModal = ({
    ...
    if ( ! namespace ) {
        throw new Error( 'Namespace is required for the ConfirmationModal component' );
    }
    ...
    return (
        <Modal
            isOpen={ isOpen }
            onClose={ onClose }
            className={ `dokan-layout max-w-xl relative z-10 bg-white ${ className || '' }` }
        >
            ...
            <Modal.Content>
                ...
                { dialogContent || (
                    <div className="sm:flex sm:items-start min-h-32">
                        {dialogIcon || (
                            <div
                                className={`flex items-center justify-center flex-shrink-0 w-14 h-14 bg-[#fff6f8] border border-[#f4e4e6] rounded-full`}>
                                <svg className={`w-4 h-4 fill-[#e64b5f] stroke-[#e64b5f]`}>
                                    <path
                ...
                { dialogFooter || (
                    <div className={ `flex items-center justify-end gap-3` }>
                        <Button
                            ...
                            className={ `inline-flex justify-center bg-white hover:bg-gray-200 text-gray-700 font-semibold text-sm py-2 px-4 border-solid border-[#e9e9e9] rounded-md` }
                        >
                            { cancelButtonText || __( 'Stay Editing', 'dokan-lite' ) }
                        </Button>
                        <Button
                            onClick={ handleConfirm }
                            loading={ isSubmitting || loading }
                            disabled={ isSubmitting || loading }
                            className={ `inline-flex justify-center bg-[#e64b5f] hover:bg-[#dc2626] text-white font-semibold text-sm py-2 px-4 border-solid border-[#e64b5f] rounded-md` }
                        >
