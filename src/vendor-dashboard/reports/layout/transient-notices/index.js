...
const QUEUE_OPTION = "dokan_analytics_transient_notices_queue";
const QUEUED_NOTICE_FILTER = "dokan_analytics_queued_notice_filter";
...
export function TransientNotices(props) {
  ...
  const {
    ...
    return {
      ...
      noticesQueue: select(OPTIONS_STORE_NAME).getOption(QUEUE_OPTION),
    };
  });
  ...
  useEffect(() => {
    getCurrentUserNotices().forEach((queuedNotice) => {
       ...
       *
       * @filter dokan_analytics_queued_notice_filter
