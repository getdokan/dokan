...
const NavigationPlugin = () => {
  ...
  const pages = getPages()
    ...
    .map((page) => {
      if (page.path === "/analytics/settings") {
        return {
          ...page,
          breadcrumbs: [__("Analytics", 'dokan-lite')],
        };
      }
      return page;
    });
  ...
  return (
    <>
      ...
      {pages.map((page) => (
        <WooNavigationItem item={page.navArgs.id} key={page.navArgs.id}>
          <Link
      ...
      {reports.map((item) => (
        <WooNavigationItem item={item.navArgs.id} key={item.navArgs.id}>
          <Link
            className="components-button"
            href={getNewPath(
