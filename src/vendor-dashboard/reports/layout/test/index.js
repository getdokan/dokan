...
jest.mock("reports/activity-panel", () => null);

jest.mock("../navigation", () => null);

jest.mock("reports/utils/admin-settings", () => {
  const adminSetting = jest.requireActual("reports/utils/admin-settings");
  return {
    ...adminSetting,
...
describe("PageLayout", () => {
  beforeEach(() => {
    jest.spyOn(window, "wpNavMenuClassChange").mockImplementation(jest.fn());
    jest.useFakeTimers();
    jest.clearAllMocks();
  });
