 ...
 */
import { FormInputProps } from "reports/utils/types";
  ...
  const countryStateOptions = countries.reduce((acc: Option[], country) => {
    ...
    const countryStates = country.states.map((state) => {
      return {
        ...
        label:
          decodeEntities(country.name) + " — " + decodeEntities(state.name),
      };
    });
  ...
  useEffect(() => {
    ...
    if (autofillCountry.length && autofillState.length) {
      filteredOptions = options.filter((option) =>
        countrySearch.test(isCountryAbbreviation ? option.key : option.label)
      );
      ...
      if (filteredOptions.length > 1) {
        filteredOptions = filteredOptions.filter(
          getStateFilter(isStateAbbreviation, normalizeState(autofillState))
        );
      }
