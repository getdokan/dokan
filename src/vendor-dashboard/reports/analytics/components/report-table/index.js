...
const TABLE_FILTER = "dokan_analytics_report_table";
...
const ReportTable = (props) => {
  ...
  const applyTableFilters = (data, totals, totalResults) => {
     ...
     *
     * @filter dokan_analytics_report_table
  ...
  const getCheckbox = (i) => {
    ...
    return {
      display: (
        <CheckboxControl onChange={partial(selectRow, i)} checked={isChecked} />
      ),
      value: false,
    };
  };
    ...
    return {
      ...
      ids:
        itemIdField && extendedTableData.items.data
          ? extendedTableData.items.data.map((item) => item[itemIdField])
          : EMPTY_ARRAY,
