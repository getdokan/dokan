...
export function dataContainsLeapYear(data) {
  if (data?.data?.intervals?.length > 1) {
    ...
    const end = data.data.intervals[data.data.intervals.length - 1].date_end;

    if (containsLeapYear && containsLeapYear(start, end)) {
      return true;
    }
  }
  return false;
...
export function buildChartData(
  ...
  for (let index = 0; index < primaryDataIntervals.length; index++) {
    ...
    let secondaryLabelDate = secondaryDateMoment.format("YYYY-MM-DD HH:mm:ss");
    let secondaryValue =
      (secondaryInterval && secondaryInterval.subtotals[selectedChartKey]) || 0;
