...
class CouponsReportTable extends Component {
  ...
  getHeadersContent() {
    return [
      ...
      {
        label: __("Amount discounted", 'dokan'),
        key: "amount",
        isSortable: true,
        isNumeric: true,
      },
      {
        label: __("Created", 'dokan'),
        key: "created",
      },
      {
  ...
  getCouponType(discountType) {
    const couponTypes = {
      percent: __("Percentage", 'dokan'),
      fixed_cart: __("Fixed cart", 'dokan'),
      fixed_product: __("Fixed product", 'dokan'),
    };
    return couponTypes[discountType] || __("N/A", 'dokan');
  }
