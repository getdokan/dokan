...
// ---------------------------------------------------------------------------
// Legacy / custom field validators (kept for backward compatibility)
// ---------------------------------------------------------------------------

/**
 * Sale price validator — kept as a named export so existing code that imports
 * it directly continues to work.  New code should prefer schema-defined
 * `validations` on the field definition instead.
 *
 * @deprecated Use `validations` in the PHP field schema instead.
 */
...
/**
 * Registry of custom field validators keyed by field id.
 *
 * Fields that define `validations` in the PHP schema are handled automatically
 * by `runSchemaValidations`; entries here are only consulted when a field has
 * *no* schema-level validations, giving a graceful fallback.
 */
