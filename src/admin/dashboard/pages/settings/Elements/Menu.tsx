...
// Sort subpages by priority.
const getSortedSubpages = ( subpages: Array< SettingsElement > ) => {
    subpages.sort( ( a, b ) => a.priority - b.priority );
    return subpages;
};
...
const Menu = ( {
    ...
    const [
        hasAutoSelectedForCurrentSearch,
        setHasAutoSelectedForCurrentSearch,
    ] = useState( false );
    const [ previousSearchText, setPreviousSearchText ] = useState( '' );
    const [ userHasManuallyNavigated, setUserHasManuallyNavigated ] =
        useState( false );
    ...
    useEffect( () => {
        ...
        const isSearchCleared =
            previousSearchText.trim() && ! searchText.trim();
        const isCompletelyNewSearch =
            searchText.trim() && ! searchText.startsWith( previousSearchText );
    ...
    const getFirstVisibleMenuAndSubmenu = useCallback( () => {
        ...
        const firstVisibleMenu = pages.find(
            ( menu ) =>
                menu.display &&
                menu.children &&
        ...
        const firstVisibleSubmenu = firstVisibleMenu.children?.find(
            ( child ) => child.display
        );
    ...
    return (
        <>
            ...
            >
                ...
                <div
                    className={ `fixed inset-y-0 left-0 w-80 max-w-sm transform transition-transform duration-300 ease-in-out flex flex-col h-full bg-white shadow-xl ${
