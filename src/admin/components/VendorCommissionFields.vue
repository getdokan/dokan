    ...
    <div class="dokan-layout">
        ...
        <div class="content-body">
            <div class="p-3">
                <div class="mb-5">
                    <label
                        class="!p-0 m-0 !mb-[6px] block"
                        for="_subscription_product_admin_commission_type"
                        >{{
                    ...
                    <div class="flex flex-col">
                        <select
                            ...
                            @change="commissionUpdated"
                            id="_subscription_product_admin_commission_type"
                            name="_subscription_product_admin_commission_type"
                            class="select short"
                        >
                            <option
                                v-for="(commissionData, key) in commissionTypes"
                                :value="key"
                            >
                                {{ commissionData }}
                            </option>
                        </select>
                        <span class="description !mt-[6px] block">{{
                            __(
                                'Set the commission type that admin will get',
                                'dokan-lite'
                            )
                        }}</span>
                    </div>
                ...
                <div
                    v-if="'category_based' === selectedCommission"
                ...
                >
                    ...
                    >
                        ...
                        <span
                            class="dokan-tooltips-help tips"
                            v-tooltip
                            :title="
                                __(
                    ...
                    <Switches
                        @input="handleResetToggle"
                        :enabled="resetSubCategory"
                    />
                </div>
                <div v-if="'category_based' === selectedCommission">
                    ...
                    >
                        ...
                        <span
                            ...
                            :title="
                                __(
                                    'When the value is 0, no commissions will be deducted from this vendor.',
                                    'dokan-lite'
                                )
                            "
                        >
...
export default {
    ...
    created() {
        ...
        if (
            this.vendorInfo.admin_commission_type &&
            Object.keys( this.commissionTypes ).includes(
                this.vendorInfo.admin_commission_type
            )
        ) {
        ...
        if ( this.vendorInfo.hasOwnProperty( 'reset_sub_category' ) ) {
            this.resetSubCategory =
                this.vendorInfo.reset_sub_category !== false;
        }
    ...
    methods: {
        handleResetToggle( value ) {
            ...
            const confirmBtnText = value
                ? this.__( 'Enable', 'dokan-lite' )
                : this.__( 'Disable', 'dokan-lite' );
        ...
        commissionUpdated() {
            this.vendorInfo.admin_additional_fee =
                this.fixedCommission.fixed ?? '';
            this.vendorInfo.admin_commission =
                this.fixedCommission.percentage ?? '';
            this.vendorInfo.admin_commission_type = this.selectedCommission;
            this.vendorInfo.admin_category_commission = JSON.parse(
                JSON.stringify( this.categoryCommission )
            );
        },
    },
};
