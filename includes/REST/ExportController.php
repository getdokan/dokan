...
class ExportController extends Controller
{
    ...
    public function export_items($request)
    {
        ...
        if (!is_wp_error($response) && !empty($response->get_data()['export_id'])) {
            $data = $response->get_data();
            $custom_filename = $this->generate_filename($request['report_args'] ?? []);
    ...
    /**
     * Rename the exported file and return the new download URL.
     *
     ...
     * @return string
     */
    protected function handle_export_file_renaming($export_id, $report_type, $download_url = '')
    {
        ...
        if (file_exists($new_path)) {
            return $download_url ? str_replace($default_filename, $new_filename, $download_url) : $download_url;
        }

        // Rename if default exists
    ...
    public function export_status($request)
    {
        ...
        if (isset($data['percent_complete']) && 100 === $data['percent_complete']) {
            $data['download_url'] = $this->handle_export_file_renaming(
                $request['export_id'],
                $request['type'],
                $data['download_url'] ?? ''
            );
