...
/**
 * Adjust product stock for a sub-order.
 *
 * @since DOKAN_SINCE
 *
...
function dokan_sub_order_adjust_product_stock( $order ) {
    ...
    // Looping through each of the line items and adjust product stock.
    foreach ( $order->get_items( 'line_item' ) as $key => $line_item ) {
        ...
        $item_order_qty = $line_item->get_quantity();
        ...
        if ( 'cancelled' === $order_status && empty( $stock_increased ) ) {
            $operation = 'increase';
            $status    = __( 'increased', 'dokan-lite' );
            ...
            wc_update_order_item_meta( $line_item->get_id(), '_dokan_stock_decreased', $item_order_qty );
            wc_update_order_item_meta( $line_item->get_id(), '_dokan_stock_increased', 0 );
        } else {
            continue;
        }
        ...
        wc_update_product_stock( $product, $item_order_qty, $operation );

        $new_stock_qty = $product->get_stock_quantity();
        $note_content  = sprintf(
        /* translators: 1: Formatted product name 2: Old stock qty 3: new stock qty 4: Stock updating status */
            __( 'Stock levels %4$s: %1$s %2$d &rarr; %3$d', 'dokan-lite' ),
