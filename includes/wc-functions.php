...
if ( ! function_exists( 'dokan_shipping_custom_split_shipping_packages' ) ) {
    /**
     ...
     * @return array
     */
    function dokan_shipping_custom_split_shipping_packages( $packages ) {
...
if ( ! function_exists( 'dokan_shipping_change_shipping_pack_name' ) ) {
    /**
     ...
     * @return string
     */
    function dokan_shipping_change_shipping_pack_name( $title, $i, $package ) {
        ...
        $shipping_label = sprintf( '%s %s', __( 'Shipping: ', 'dokan-lite' ), ! empty( $store_info['store_name'] ) ? $store_info['store_name'] : '' );

        return apply_filters( 'dokan_shipping_package_name', $shipping_label, $i, $package );
    }
}

if ( ! function_exists( 'dokan_shipping_add_shipping_pack_meta' ) ) {
    /**
     ...
     * @return void
     */
    function dokan_shipping_add_shipping_pack_meta( $item, $package_key, $package, $order ) {
        $item->add_meta_data( 'seller_id', $package['seller_id'], true );
    }
}

// If dokan pro is not active Only then split shipping from lite.
if ( ! function_exists( 'dokan_pro' ) ) {
    add_filter( 'woocommerce_cart_shipping_packages', 'dokan_shipping_custom_split_shipping_packages' );
    add_action( 'woocommerce_checkout_create_order_shipping_item', 'dokan_shipping_add_shipping_pack_meta', 10, 4 );
    add_filter( 'woocommerce_shipping_package_name', 'dokan_shipping_change_shipping_pack_name', 10, 3 );
}
