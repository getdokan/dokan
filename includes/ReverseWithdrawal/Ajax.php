...
class Ajax {
    ...
    public function reverse_withdrawal_payment() {
        ...
        if ( ! isset( $_POST['_reverse_withdrawal_nonce'] ) || ! wp_verify_nonce( sanitize_key( wp_unslash( $_POST['_reverse_withdrawal_nonce'] ) ), 'dokan_reverse_withdrawal_payment' ) ) {
            wp_send_json_error( [ 'message' => __( 'Invalid nonce', 'dokan-lite' ) ], 400 );
        }

        // check permission, don't let vendor staff view this section
        if ( ! current_user_can( 'dokandar' ) ) {
            wp_send_json_error( [ 'message' => __( 'You do not have permission to use this action.', 'dokan-lite' ) ], 400 );
        }
        ...
        if ( ! $product ) {
            wp_send_json_error( [ 'message' => __( 'Invalid base payment product found. Please contact with site admin.', 'dokan-lite' ) ], 400 );
        }

        if ( $product->get_status() !== 'publish' ) {
            wp_send_json_error( [ 'message' => __( 'Base payment product status is not published. Please contact with site admin.', 'dokan-lite' ) ], 400 );
        }
        ...
        try {
            ...
            wp_send_json_error( [ 'message' => __( 'Something went wrong while adding product to cart.', 'dokan-lite' ) ], 400 );
        }
