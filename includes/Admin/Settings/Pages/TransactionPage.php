...
class TransactionPage extends AbstractPage {
    ...
    public function option_dispatcher( $data ): void {
        ...
        // Handle reverse withdrawal settings
        if ( isset( $data['reverse_withdrawal']['reverse_withdrawal_section'] ) ) {
            $reverse_withdrawal_data = $data['reverse_withdrawal']['reverse_withdrawal_section'];
            $dokan_reverse_withdrawal = get_option( 'dokan_reverse_withdrawal', [] );

            $dokan_reverse_withdrawal['enabled']                   = $reverse_withdrawal_data['enabled'] ?? 'off';
            $dokan_reverse_withdrawal['billing_type']              = $reverse_withdrawal_data['billing_type'] ?? 'by_amount_limit';
            $dokan_reverse_withdrawal['reverse_balance_threshold'] = $reverse_withdrawal_data['reverse_balance_threshold'] ?? 150;
            $dokan_reverse_withdrawal['monthly_billing_day']       = $reverse_withdrawal_data['monthly_billing_day'] ?? 1;
            $dokan_reverse_withdrawal['due_period']                = $reverse_withdrawal_data['due_period'] ?? 7;
            $dokan_reverse_withdrawal['failed_actions']            = $reverse_withdrawal_data['failed_actions'] ?? [ 'enable_catalog_mode' ];
            $dokan_reverse_withdrawal['display_notice']            = $reverse_withdrawal_data['display_notice'] ?? 'on';

            $dokan_reverse_withdrawal = apply_filters( 'dokan_admin_reverse_withdrawal_settings', $dokan_reverse_withdrawal );
            update_option( 'dokan_reverse_withdrawal', $reverse_withdrawal_data, $dokan_reverse_withdrawal );
        }
    }
}
