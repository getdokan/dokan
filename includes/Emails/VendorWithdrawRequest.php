...
class VendorWithdrawRequest extends WC_Email {
    ...
    public function trigger( $withdraw ) {
        ...
        // Format amounts using wc_format_decimal() to respect WooCommerce "Number of decimals" setting
        $this->placeholders['{amount}']        = dokan()->email->currency_symbol( wc_format_decimal( $amount, '' ) );
        $this->placeholders['{charge}']        = dokan()->email->currency_symbol( wc_format_decimal( $withdraw_charge, '' ) );
