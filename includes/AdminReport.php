 ...
 *
 * This class is a copy of WC_Admin_Report from `WC()->plugin_path() . '/includes/admin/reports/class-wc-admin-report.php'`
 *
 * @package Dokan\Admin\Reports
 */
...
class AdminReport {
	...
	public function get_order_report_data( $args, $order_table = false  ) {
        ...
        $query['from']   = "FROM {$table} AS posts";
        ...
        foreach ( ( $data + $where ) as $raw_key => $value ) {
            ...
            switch ( $type ) {
                case 'meta':
                    $joins[ "meta_{$key}" ] = "{$join_type} JOIN {$meta_table} AS meta_{$key} ON ( posts.{$id} = meta_{$key}.{$meta_table_join_key} AND meta_{$key}.meta_key = '{$raw_key}' )";
                    break;
                case 'parent_meta':
                    $joins[ "parent_meta_{$key}" ] = "{$join_type} JOIN {$meta_table} AS parent_meta_{$key} ON (posts.{$post_parent} = parent_meta_{$key}.{$meta_table_join_key}) AND (parent_meta_{$key}.meta_key = '{$raw_key}')";
                    break;
                case 'order_item_meta':
        ...
        if ( ! empty( $where_meta ) ) {
            foreach ( $where_meta as $value ) {
                ...
                if ( 'order_item_meta' === $type ) {
                    ...
                    $joins[ "meta_{$key}" ] = "{$join_type} JOIN {$meta_table} AS meta_{$key} ON posts.{$id} = meta_{$key}.{$meta_table_join_key}";
                }
            }
        }

        if ( ! empty( $parent_order_status ) ) {
            $joins['parent'] = "LEFT JOIN {$table} AS parent ON posts.{$post_parent} = parent.{$id}";
        }

        $query['join'] = implode( ' ', $joins );

        $post_type = $order_table ? 'type' : 'post_type';

        $query['where'] = "
			WHERE 	posts.{$post_type} 	IN ( '" . implode( "','", $order_types ) . "' )
			";

        $post_status = $order_table ? 'status' : 'post_status';
        ...
        if ( $filter_range ) {
            $date_column = $order_table && OrderUtil::is_hpos_enabled() ? 'date_created_gmt' : 'post_date';
            $query['where'] .= "
				AND 	posts.{$date_column} >= '" . date( 'Y-m-d H:i:s', $start_date ) . "'
				AND 	posts.{$date_column} < '" . date( 'Y-m-d H:i:s', strtotime( '+1 DAY', $end_date ) ) . "'
			";
        }
